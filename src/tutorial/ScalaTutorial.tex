...
\renewcommand{\docsubtitle}{Version 1.1}
...
object representing the console, with the friendly greeting as
argument. The \code{main} method does not return a value (it is a
procedure method). Therefore, it is not necessary to declare a return
type.
...
almost, but \code{==} has one advantage over \code{equals} in that it
works also when the selector is the \code{null} constant.

The \code{fact} method also shows some interesting properties of
\Scala's syntax. The first one is that a method body (such as the body
of \code{fact}) does not have to be surrounded by curly braces if it
...
\end{lstlisting}
Note that in order to print the string, we used method \code{println}
of class \code{Console} instead of using the one from
