...
\subsection{Method Type Inference}\label{sec:meth-type-inf}
...
in a base class of $C$ may also leave out the return type, even if it
is recursive. In this case, the return type $R'$ of the overridden
...
\commadots U_n]$ where some of the $U_i$ are types instead of type
variable patterns or wildcards. However, such a type pattern
will be translated to the erasure (\sref{sec:erasure}) of $T[U_1
\commadots U_n]$.  The Scala compiler will issue an ``unchecked''
warning for these patterns to flag the possible loss of type-safety.
