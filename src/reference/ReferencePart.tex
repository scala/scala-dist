...
{\em monomorphic}.
\item[] $as$ is a a sequence of annotations (\sref{sec:annotations}) such as
annotations are given, they apply to the primary constructor
to the class.
...
Such warnings can be suppressed by following the \code{match} keyword 
with an explanation mark, as in
\begin{lstlisting}
e match! { case $p_1$ => $b_1$ $\ldots$ case $p_n$ => $b_n$ }
\end{lstlisting}
...
\syntax\begin{lstlisting}
  Annotation      ::=  `@' AnnotationExpr [NewLine]
...
e: @unsealed                          // An expression annotation
\end{lstlisting}
...
annotation causes getter and setter methods \code{getX}, \code{setX} in
