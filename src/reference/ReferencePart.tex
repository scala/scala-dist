...
floatType            ::=  'F' | 'f' | 'D' | 'd'
\end{lstlisting}
Foating point literals are of type \lstinline@float@ when followed by
...
same scope as well as bindings of the same or lower preceence in outer
scopes.
...
bounds}, i.e.\ $L_i\sigma <: T_i <: U_i\sigma$ where $\sigma$ is the
...
\begin{lstlisting}
  class TreeMap[a <: Ordered[a], b] { $\ldots$ }
  class List[a] { $\ldots$ }
  class I extends Ordered[I] { $\ldots$ }
\end{lstlisting}
...
\begin{lstlisting}
package scala 
mixin class Function$n$[-$T_1 \commadots$ -$T_n$, +$R$] {
...
If the lower bound $L$ is missing, the bottom type
...
vary with respect to subtyping (\sref{sec:subtyping}).  A
...
\syntax\begin{lstlisting}
  TmplDef          ::=  ([case | mixin] class) ClassDef
...
Each mixin class type reference $mt_i$ must mention a class that has the
...
that case the list of parants is implicitly extended to include the
...
becomes
\begin{lstlisting}
$mc_1$ with $\ldots$ with $mc_n$ {$\stats\,$} with ScalaObject .
\end{lstlisting}
...
\begin{lstlisting}
mixin class AbsIterator extends AnyRef with ScalaObject { ... }
...
Consider a class $C$ defined by the template 
~\lstinline@$sc$ with $mc_1$ with $mc_n$ {$\stats\,$}@. 

If $C$ is a mixin class then it is mixin-evaluated by evaluating the
statement sequence $\stats$.

if $C$ is not a mixin class, then an evaluation of $C$ consists of the
following steps.
\begin{itemize}
\item
First, the superclass constructor $sc$ is evaluated.
\item
Then, all base classes in the template's linearization up to the
template's superclass denoted by $sc$ are
mixin-evaluated. Mixin-evaluation happens in reverse order of
occurrence in the linearization, i.e. the class immediatetly preceding
...
It is an error if a block contaions

\end{definition}
Member definitions fall into two categories: concrete and abstract.
Members of class $C$ are either directly defined (i.e. they appear in
$C$'s statement sequence $\stats$ or inherited.  There are two rules
...
\item
If $M$ is labelled ~\lstinline@private[$C$]@~ for some enclosing class or package $C$,
then $M'$ must be labelled ~\lstinline@private[$C'$]@~ for some class or package $C'$ where
...
labelled \code{abstract override}.
\end{itemize}

\example\label{ex:compound-a}
Consider the definitions:
\begin{lstlisting}
mixin class Root { type T <: Root }
mixin class A extends Root { type T <: A }
mixin class B extends Root { type T <: B }
mixin class C extends A with B 
\end{lstlisting}
...
Members labelled with such a modifier can be accessed only from code
...
We say, a member {\em is private} if it is labelled with a
...
is only allowed for value members of mixin classes.  A member labelled
...
If an \code{override} modifier is given, there must be at least one
overridden member definition or declaration (either concrete or
abstract).
...
member for which only a declaration is given is abstract, whereas a
member for which a full definition is given is concrete.
\item
...
\code{sealed} class may not be inherited, except if the inheriting 
...
~\lstinline@class $c$[$\tps\,$]($\ps\,$) requires $s$ extends $t$@.
Here,
\begin{itemize}
\item[]
...
or \code{var} keyword, an accessor (getter) definition for this
parameter is implicitly added to the class. The getter introduces a
value member $x$ of $c$ that is defined as alias of the parameter. If
the introducing keyword is \code{var}, a setter accessor $x_=$ is
also implictly added to the class. In invocation of that setter
$x_=(e)$ changes the value of the paarameter to the result of
evaluating $e$.  The formal parameter declaration may contain
modifiers, which then carry over to the accessor definition(s).

If a formal parameter declaration $x: T$ is preceded by 
\item[] 
...
\lstinline@<=@, \lstinline@>@, and \lstinline@>=@. It is similar, but not identical to class \code{Ordered} in the standard Scala library.

\begin{lstlisting}
trait Ordered[t <: Ordered[t]] requires t { 
  def < (that: t): Boolean
...
\syntax\begin{lstlisting}
  ObjectDef       ::=  id [`requires' SimpleType] ClassTemplate
\end{lstlisting}
...
\begin{lstlisting}
final class $m\Dollar$cls requires $s$ extends $t$ 
...
compatibe with every reference type.  It denotes a reference value
...
\syntax\begin{lstlisting}
  Expr1          ::=  if `(' Expr `)' Expr [[StatementSeparator] else Expr]
\end{lstlisting}
...
further generators, value definitions, or filters.  A {\em generator}
~\lstinline@val $p$ <- $e$@~ produces bindings from an expression $e$
which is matched in some way against pattern $p$. A {\em value
definition} binds a value name (or several names in a pattern) to the
result of evaluating an expression. A {\em filter} is an expression
which restricts enumerated bindings. The precise meaning of generators
and filters is defined by translation to invocations of four methods:
\code{map}, \code{filter}, \code{flatMap}, and \code{foreach}. These methods can
...
Finally, here is a function to compute the product of two matrices. Compare with the imperative version of \sref{ex:imp-mat-mul}.
\begin{lstlisting}
def matmul(xss: Array[Array[double]], yss: Array[Array[double]]) = {
...
\item $p$ is a constructor pattern $c(p_1 \commadots p_n)$, the type $T$
      is an instance of class $c$, the primary constructor of type $T$ has
...
\section{Package Clauses}\label{sec:package-clauses}
