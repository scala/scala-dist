...
(\sref{sec:object-defs}). Qualified private members are inherited only
from templates inside $C$.

We say, a member {\em is private} if it is labeled with a
\code{private} modifier without qualification.  Unqualified
\code{private} may not be applied to abstract members, and it may not
be combined in one modifier list with \code{protected}, \code{final}
or \code{override}.
\item
...
i.e.\ \lstinline@{$e$,}@. Finally, the empty tuple ~\lstinline@{}@~
is an alias for the unit value ~\lstinline@()@.
...
A {\em variable pattern} $x$ is generally a simple identifier which
starts with a lower case letter.  It matches any value, and binds the
variable name to that value.  The type of $x$ is the expected type of
the pattern as given from outside.  A special case is the wild-card
...
In this case the pattern matches only the single value designated by
$p$ (i.e. the pattern match amounts to a test whether the selector
...
$c(v_1 \commadots v_n)$ where each element pattern $p_i$ matches the
corresponding value $v_i$.

A {\em tuple pattern} \lstinline@{$p_1 \commadots p_n$}@ is an alias
for the constructor pattern 
~\lstinline@scala.Tuple$n$($p_1 \commadots p_n$)@, where $n \geq 2$.  
The pattern may also be written with a
trailing comma, i.e.\ \lstinline@{$p_1 \commadots p_n$,}@.  Unary
tuple patterns can be expressed in this syntax only by using a trailing comma,
i.e.\ \lstinline@{$p$,}@. Finally, the empty tuple \lstinline@{}@ is
an alias for the unit value \lstinline@()@.
...
\subsection{Type Patterns}\label{sec:type-patterns}
...
\subsection{Type Parameter Inference in Patterns}\label{sec:type-param-inf-pat}
