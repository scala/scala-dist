...
using a trailing comma, i.e. ~\lstinline@{$T$,}@. Finally, the empty
tuple type ~\lstinline@{}@~ is an alias for type
~\lstinline@scala.Unit@.
...
\syntax\begin{lstlisting}
  Type ::= CompoundType `=>' Type | `(' [Types | `=>' Type] `)' `=>' Type
\end{lstlisting}
...
The type ~\lstinline@(=> $T$) => $U$@~ represents functions with a 
call-by-name parameter (\sref{sec:by-name-params}) of type $T$ which yield
a result of type $U$.
...
\end{itemize}
References to the type parameters in object-private members of the
class are not checked for their variance position. In an
object-private member, the type parameter may appear anywhere without
...
within the directly enclosing template and its companion module.  They
...
class-local}, whereas members labeled with \lstinline@private[this]@
are called {\em object-local}.  A member {\em is private} if it is
either class-local or object-local, but not if it is marked
\lstinline@private[$C$]@ where $C$ is an identifier; in the latter
case the member is called {\em qualified private}.

Class-local or object-local \code{private} may not be applied to
abstract members, and it may not be combined in one modifier list with
...
\begin{lstlisting}
class $c$[$\tps\,$]$m$($\ps_1$)$\ldots$($\ps_n$) requires $s$ extends $t$    $\gap(n \geq 0)$.
\end{lstlisting}
Here,
\begin{itemize}
\item[]
...
An alternative notation \code{()}  for the same value is deprecated
after Scala version 2.3.2. 
...
to a function type by eta expansion (\sref{sec:eta-expand}).
...
~\lstinline@x: T*@ (\sref{sec:repeated-params}), because its result type
...
A constructor pattern is of the form $c(p_1 \commadots p_n)$
where $n \geq 0$. It consists of a stable identifier $c$, followed by
element patterns $p_1 \commadots p_n$. The constructor $c$ is a simple
or qualified name which denotes a case class
...
\syntax\begin{lstlisting}
  SimplePattern   ::=   `{' [Pattern `,' [Patterns [`,']]] `}'
\end{lstlisting}
...
\item[]
$n=1$ and \code{unapply}'s result type is \code{Option[$T$]}, for some
...
In this case, the last element pattern may be a {\em sequence
wildcard} \code{_*}.  Each element pattern $p_i$ is type-checked with
$T$ as expected type, unless it is a sequence wildcard. If a final
sequence wildcard is present, the pattern matches all values $v$ that
are sequences of class $S$ which start with elements matching patterns
$p_1 \commadots p_{n-1}$.  If no final sequence wildcard is given, the
pattern matches all values $v$ that are sequences of class $s$ and of
...
A sequence pattern $s(p_1 \commadots p_n)$ where $n \geq 0$
...
Sequence patterns are deprecated after Scala version 2.3.2. It is
recommended to replace them with extractor patterns.
...
of type \lstinline@scala.Array$[U_1]$@, where $U_1$ is a type is matched by $T_1$.
\end{itemize}
...
EmptyElemTag  ::=    `<' Name {S XMLAttribute} [S] `/>'                         

STag          ::=    `<' Name {S XMLAttribute} [S] `>'                          
ETag          ::=    `</' Name [S] '>'                                        
...
\syntax\begin{lstlisting}
  ...
  Attribute       ::=  Constr [`{' {NameValuePair} `}']
...
$a_1 \commadots a_n$ must be constant expressions. The constructor may
be optionally followed by a list of name/value pairs in braces, e.g.
~\lstinline@{$n_1 = c_1 \commadots n_k = c_k$}@. All values $c_i$ in that
list must be constant expressions. 

An attribute clause applies to the first definition or declaration
following it. More than one attribute clause may precede a definition
and declaration. The order in which these clauses are given does not
matter.  It is also possible to combine several attributes separated
by commas in one clause. Such a combined clause $[A_1 \commadots A_n]$
is equivalent to a set of clauses $[A_1] \ldots [A_n]$.
...
Other attributes may be interpreted by platform- or
application-dependent tools. Class \code{scala.Attribute} has two
sub-traits which are used to indicate how these attributes are
...
in every compilation unit where the attributed symbol is accessed. An
attribute class can inherit from both \code{scala.ClassfileAttribute}
and \code{scala.StaticAttribute}. If an attribute class inherits from
neither \code{scala.ClassfileAttribute} nor
\code{scala.StaticAttribute}, its instances are visible only locally
