...
first the set of applicable alternatives. A method alternative is
...
it.  A polymorphic method is applicable if local type inference
can determine type arguments so that the instantiated method is
applicable.
...
following definition of being ``as specific as'', and ``more specific than'':
\begin{itemize} 
\item
A parameterized method of type \lstinline@($\Ts\,$)$U$@ is as specific as some other
member of type $S$ if $S$ is applicable to arguments \lstinline@($\ps\,$)@ of
types $\Ts$.
\item
...
trailing comma, i.e.\ \lstinline@($p_1 \commadots p_n$,)@.
The empty tuple
\lstinline@()@ is the unique value of type \lstinline@scala.Unit@.
...
\begin{quote} Marks a definition as deprecated. Accesses to the
  ...
  deprecated warning to be issued from the compiler.  Deprecated
...
%% @@@KSW If X is a boolean, is the getter called isX?
%% MO: I don't think so.
