...
also all \code{implicit} members of a companion module
(\sref{sec:object-defs}) of a class which is associated with the
implicit parameter's type.

Here, we say a class $C$ is {\em associated} with a type $T$, if it is
a base class (\sref{sec:linearization}) of some part of $T$.  The {\em
parts} of a type $T$ are:
\begin{itemize}
\item
if $T$ is a compound type ~\lstinline@$T_1$ with $\ldots$ $T_n$@, the
...
$T$. A special case is if type $T$ is a singleton type, say $p.type$.
...
value is equal to $p$ using method \code{eq} in class \code{AnyRef}).
