...
definitions and declarations (\sref{sec:defs}), inheritance (\sref{?})
import clauses (\sref{sec:import}), or package clauses
(\sref{sec:package-clauses}) which are collectively called {\em
bindings}.
...
\end{lstlisting}
Hence, function types are covariant (\sref{variances}) in their result type and
...
\syntax\begin{lstlisting}
  ...
  Def             ::=  type TypeDef
  TypeDef         ::=  id [NewLine] [TypeParamClause] `=' Type
\end{lstlisting}
...
\lstinline@scala.All@ is assumed.  If the upper bound $U$ is absent,
...
FunSig             ::=  id [NewLine] [FunTypeParamClause] ParamClauses
...
which defines the template's {\em superclass}, mixin class type
references ~\lstinline@$mt_1 \commadots mt_n$@~ $(n \geq 0)$, which
define the template's {\em mixin classes}, and a statement sequence
$\stats$ which contains initialization code and additional member definitions
for the template.

Each mixin class type reference $mt_i$ must denote a class that has the
\code{mixin} modifier set.  By contrast, the superclass constructor $sc$ 
normally refers to a class which is not a mixin class. It is possible
to write a list of parents that starts with a mixin class type
reference, e.g.  ~\lstinline@$mt_1$ with $\ldots$ with $mt_n$@. In
...
at least one constructor that does not take parameters.  In the
following, we will always assume that this implicit extension has been
performed, so that the first parent class of a template is a regular
superclass constructor, not a mixin class type reference.

The list of parents of every class is also always implicitly extended
by a reference to the \code{scala.ScalaObject} mixin class type as last mixin class. E.g.\ 
\begin{lstlisting}
...
\end{lstlisting}
In this case, the definition of \code{Sub} is expanded to:
\begin{lstlisting}
...
its superclass and Java interfaces as its mixin classes. 

\paragraph{\em Template Evaluation}
Consider a template ~\lstinline@$sc$ with $mt_1$ with $mt_n$ {$\stats\,$}@.

If this is the template of a mixin class (\sref{sec:mixin-classes}) then its {\em
mixin-evaluation} consists of an evaluation of the statement sequence
$\stats$.

If this is not a template of a mixin class, then its {\em evaluation}
...
~\lstinline@$x$.$c$[$\targs$]($\args\,$)@, where $x$ is a stable identifier
...
list, and $\args$ is an argument list, which matches one of the
...
using local type inference (\sref{sec:local-type-inf}). The argument list
\lstinline@($\args\,$)@ can also be omitted, in which case an empty
argument list \lstinline@()@ is implicitly added.

An evaluation of a constructor invocation 
~\lstinline@$x$.$c$[$\targs$]($\args\,$)@~
consists of the following steps:
\begin{itemize}
\item First, the prefix $x$ is evaluated.
\item Then, the arguments $\args$ are evaluated from left to right.
...
is only allowed for value members of mixin classes.  A member labeled
...
mandatory if the class has incomplete members, yet is not a mixin
class.  Abstract classes cannot be instantiated
(\sref{sec:inst-creation}) with a constructor invocation unless
followed by mixin types and/or a refinement which override all incomplete
...
\label{sec:mixin-classes}

\syntax\begin{lstlisting}
  TmplDef          ::=  mixin class MixinClassDef
  MixinClassDef    ::=  id [NewLine] [TypeParamClause]
                        [`requires' SimpleType] [extends MixinParents] 
                        [[NewLine] TemplateBody]
  MixinParents     ::=  SimpleType {`with' SimpleType}
\end{lstlisting}

A mixin class is a class that is meant to be added to some other class
as a mixin. Unlike normal classes, mixin classes cannot have
constructor parameters. Furthermore, no constructor arguments are
passed to its superclass. This is not necessary as mixins are
initialized after the superclass is initialized.

Assume a mixin class $M$ defines some aspect of an instance $x$ of
type $C$ (i.e.\ $M$ is a base class of $C$). Then the {\em actual
supertype} of $M$ in $x$ is the compound type consisting of all the
base classes in $\lin C$ that succeed $M$.  The actual supertype gives
the context for resolving a \code{super} reference in a mixin class
(\sref{sec:this-super}). Note that the actual supertype depends 
on the type to which the mixin is added in a composition; it is not
statically known at the time the mixin is defined.

If $B$ is not a mixin class, then its actual supertype is simply its
...
\end{lstlisting}
Here is a mixin class that prevents concurrent access to the
\code{get} and \code{set} operations of its parent class:
\begin{lstlisting}
mixin class SynchronizedTable[A, B] extends Table[A, B] {
...
\syntax\begin{lstlisting}
  TmplDef ::= trait MixinClassDef
\end{lstlisting}

A trait is a stateless mixin class.  All non-empty statements in the
trait's template must be either imports or pure definitions. They may
not be variable definitions. 

A {\em pure} definition can be evaluated without any side effect.
Function, type, and class definitions are always pure. A value
definition is pure if its right-hand side expression is pure. Pure
expressions are paths, literals, typed expressions $e: T$ where $e$ is
pure, and blocks consisting only of pure definitions and a pure
...
\syntax\begin{lstlisting}
  ...
  Expr1           ::=  if `(' Expr `)' [NewLine] Expr [[`;'] else Expr]
                    ...
                    |  for `(' Enumerators `)' [NewLine] [yield] Expr
                    |  return [Expr]
                    |  throw Expr
                    |  [SimpleExpr `.'] id `=' Expr
...
The \code{null} value is of type \lstinline@scala.AllRef@, and is thus
...
\end{lstlisting}
The linearization of class \code{E} is ~\lstinline@{E, D, C, B, A, Root}@.
Then we have:
\begin{lstlisting}
(new A).superA == "Root", (new B).superB == "Root"
(new C).superA == "Root", (new C).superB == "A", (new C).superC == "B"
(new D).superA == "Root", (new D).superD == "A"
(new E).superA == "Root", (new E).superB == "A", (new E).superC == "B",
   (new E).superD == "C", (new E).superE == "D"
\end{lstlisting}
...
\example Consider the following definitions of arithmetic terms:
...
\chapter{Top-Level Definitions and Programs}
\label{sec:topdefs}
...
\section{Packagings}\label{sec:package-clauses}

\syntax\begin{lstlisting}
  Packaging       ::=  package QualId `{' {TopStat StatementSeparator} TopStat `}'
\end{lstlisting}
...
every user-defined Scala class also inherits the mixin class
...
The test \code{$x$.asInstanceOf[$T$]} is treated specially if $T$ is a
...
when required by the context (\label{sec:impl-conv}).
...
Any numeric value type $T$ supports the support the following methods.
\begin{itemize}
\item 
...
overloaded alternatives. Each alternative takes an instance $x$ of
some numeric value type as argument. Its result type is type
\code{Boolean}. The operation is evaluated by converting the receiver
and its argument to their operation type and performing the given
comparison operation of that type.
\item
...
alternative takes an instance $x$ of some numeric value type $U$ as
argument. Its result type is the operation type of $T$ and $U$. The
operation is evaluated by converting the receiver and its argument to
their operation type and performing the given arithmetic operation of
that type.
\item
...
overloaded alternatives. Each alternative takes an instance $x$ of
some integer numeric value type $U$ as argument. Its result type is 
the operation type of $T$ and $U$. The
operation is evaluated by converting the receiver and its argument to
their operation type and performing the given arithmetic operation of
that type.
\item
...
methods of has two overloaded alternatives, which take an argument $x$
...
The operation is evaluated by converting the receiver to the result
type and performing the specified shift by $x$ bits.
\end{itemize}

Numeric value types implements operations \code{equals},
...
is appropriate for that type. That is, \code{equals}{ can be thought
of being defined in a numeric value type $T$ as follows:
\begin{lstlisting}
...
numeric values to equal results. It is guranteed to be the identity for 
...
The \lstinline@String@ class is usually derived from the standard String
...
(\sref{cls:seq}) by defining an \code{elements} method which returns
...
object Predef {
  ...
  def Array[A <: AnyRef](xs: A*): Array[A] = val array = new Array[A](xs.length)
    var i = 0
  ...
  implicit def seq2ordered[A <# Ordered[A]](xs: Array[A]): Ordered[Seq[A]] = new Ordered[Seq[A]] with Proxy {
    def compareTo[B >: Seq[A] <# Ordered[B]](that: B): Int = that match {
      case that: Seq[A] =>
        ...
        val those = those.elements
        while (res == 0 && these.hasNext)
          res = if (!those.hasNext) 1
                else these.next compareTo those.next
      case _ =>
        - (that compareTo xs)
  }
