...
Definitions and declarations have the highest precedence, no
...
\lstinline@$C$.this.type#$t$@, except if the type $t$ occurs as a part
of a typed pattern (\sref{sec:typed-patterns}). In the latter case,
$t$ is taken as a shorthand for just \lstinline@$C$#$t$@.  If $t$ is
...
parameter bindings such as \lstinline@$x$: $T = e$@, which bind value
...
The scope of a formal value parameter name $x$ is all subsequent parameter
clauses and the function body, if one is given. Both type parameter names
...
constructor of that class which is applicable (\sref{sec:overloading-resolution})
...
same transformation as described in section \sref{sec:apply}.
...
not have \code{protected}, \code{final} or \code{override} modifiers.
\item
...
parameter sections and the the constructor
...
has some value type, the application is taken to be equivalent to
\lstinline@$f$.apply($e_1 \commadots e_m$)@, i.e.\ the application of
an \code{apply} method defined by $f$. If $f$ has a method type
\lstinline@($p_1:T_1 \commadots p_n:T_n$)$U$@, $f$ must be applicable
(\sref{sec:overloading-resolution}) to $e_1 \commadots e_m$.

If the application uses either named or default arguments the following
transformation is applied to convert it into an application without named or
default arguments. If the function $f$ has the form \lstinline@$p.m$[$\targs$]@
...
If the return expression is itself part of an anonymous function, it is possible
...
\code{Byte}, \code{Short} or \code{Char}, and
...
in \lstinline@$e$($e_1 \commadots e_m$)@. For every member $m$ in $\AA$ one
determines whether it is {\em applicable}. If $m$ has a method
type $(p_1:T_1 \commadots p_n:T_n)U$ we say that an argument expression
$e_i$ is a {\em named} argument if it has the form $x_i=e'_i$ and
$x_i$ is one of the parameter names $p_1 \commadots p_n$. The member
$m$ is applicable if there are not too many arguments ($m \le n$) and all
of the follwing conditions hold:
\begin{itemize}
...
\item The names $x_i$ of all named arguments are pairwise distinct and no named
  argument defines a parameter which is already specified by a positional argument.
\item For every positional argument $e_i$ the type obtained by typing $e_i$
  with an undefined expected type is compatible with $T_i$.
\item For every named argument $x_i=e'_i$ the type obtained by typing
  $e'_i$ with an undefined expected type is compatible with the parameter type
  $T_j$ whose name $p_j$ matches $x_i$.
\item Every parameter $p_j:T_j$ which is not specified by either a positional
  or a named argument has default argument.
\item If the expected type is defined, the result type $U$ is compatible to it.
\end{itemize}

If $m$ is a polymorphic method it is applicable if local type inference can
determine type arguments so that the instantiated method is applicable. If
$m$ has some value type it is applicable if it has a method member named
...
\begin{itemize} 
\item
A parameterized method of type \lstinline@($\Ts\,$)$U$@ is as specific as some other
member $m'$ of type $S$ if $m'$ is applicable to arguments \lstinline@($\ps\,$)@ of
types $\Ts$.
\item
A polymorphic method of type
~\lstinline@[$a_1$ >: $L_1$ <: $U_1 \commadots a_n$ >: $L_n$ <: $U_n$]$T$@~ is
as specific as some other member of type $S$ if $T$ is as %% "as specifc" is on members, but here it's used on types (T / S).
...
An alternative $A$ is {\em more specific} than an alternative
$B$ if $A$ is as specific as $B$, and $B$ is also as specific as $A$.

It is an error if there is no alternative in $\BB$ which is more
...
which is defined in a proper subclass of $C$.
...
\item A class constructed with \code{classOf} (\sref{sec:cls:predef})
