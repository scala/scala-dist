...
Analogously, for any sequence of expressions or
patterns $x_1 \commadots x_n$, 
...
\paragraph{Infix operators of greater arities}
...
using \code{unapply} methods in extractor objects. Here is an example:
\begin{lstlisting}
object Twice {                              
  def apply(x:Int) = x*2
...
in a sense the reverse of \code{apply}. \@unapply@ methods return option types: 
...
the \code{unapply} method of object \code{Twice}. The two last lines of the example
above are translated as follows. More details on extractors can be found
in the paper ``Matching Objects with Patterns'' by Emir, Odersky and Williams.
 
\paragraph{Deprecated Attribute}
