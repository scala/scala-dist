...
To illustrate what is really happening, we can use this mockup
...
\end{lstlisting}
The output when running a program that accesses one employee 
...
class Symbols(val compiler: Compiler) {
  ...
  val Int_Add = new Symbol("+", FunType(List(IntType, IntType), IntType))
  val Int_Sub = new Symbol("-", FunType(List(IntType, IntType), IntType))
