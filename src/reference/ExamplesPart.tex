...
\begin{lstlisting}[style=floating,label=fig:simple-auction-msgs,caption=Message Classes for an Auction Service]
...
limits this to tuples of some reasonable number of elements).  Also,
Scala defines an alias
...
def queens(n: int): List[List[int]] = {
  def placeQueens(k: int): List[List[int]] =
    if (k == 0) List(List())
               ...
               isSafe(column, queens, 1) } yield column :: queens
  placeQueens(n)
}
\end{lstlisting}
...
matrix represented as a list of lists:
 \begin{lstlisting}
for (val xs <- xss) {
  for (val x <- xs) System.out.print(x + "\t")
  System.out.println()
}
\end{lstlisting}
...
inserts the pair \lstinline@{curtime + delay, action}@ into the
...
\begin{lstlisting}
import Iterator._
for (val {x, i} <- fromArray(xs) zip from(0); x > limit)
yield i
\end{lstlisting}
...
trait ListParsers extends CharParsers {
  def ident: Parser[Tree] = 
    for {
      val c: char <- chrSuchThat(Character.isLetter)
      val cs: List[char] <- rep(chrSuchThat(Character.isLetterOrDigit))
...
\begin{lstlisting}
  def number: Parser[Tree] =
    for {
      val d: char <- chrSuchThat(Character.isDigit)
      val ds: List[char] <- rep(chrSuchThat(Character.isDigit))
...
trait MiniMLParsers extends CharParsers {
  ...
  def id: Parser[String] = 
    for {
      val c: char <- whitespace &&& chrSuchThat(Character.isLetter); 
      val cs: List[char] <- rep(chrSuchThat(Character.isLetterOrDigit))
    } yield (c :: cs).mkString("", "", "")
