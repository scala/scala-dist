...
There is a new marker trait \@scala.NotNull@. Types inheriting from
that trait do not have \@null@ in their set of legal values.
The type \@scala.Null@ conforms only to class or trait types which do 
not inherit from \@NotNull@. Furthermore, a reference to a class or object member
whose type conforms to \@NotNull@ always checks that the member is
