...
\section{Context Bounds}\label{sec:context-bounds}
...
A method or class containing type parameters with context bounds is treated as being
equivalent to a method with implicit parameters. Consider first the case of a
single parameter with context bounds such as:
\begin{lstlisting}
def $f$[$A$ : $T_1$ : $T_n$]($\ps$): $R$ = ...
\end{lstlisting}
Then the method definition above is expanded to
\begin{lstlisting}
def $f$[$A$]($\ps$)(implicit $ev_1$: $T_1$[$A$], ..., $ev_n$: $T_n$[$A$]): $R$ = ...
\end{lstlisting}
where the $ev_i$ are fresh names for the newly introduced implicit parameters. These
parameters are called {\em evidence parameters}.

If a class or method has several context-bounded type parameters, each
...
Consequently, type-parameters in traits may not be context-bounded.
...
\newpage
\section{Java Platform Annotations}
...
\section{Java Beans Annotations}

\lstinline^@scala.beans.BeanProperty^
\begin{quote}
...
\lstinline^@scala.beans.BooleanBeanProperty^
\begin{quote}
...
\section{Deprecation Annotations}

\lstinline^@deprecated(message: <stringlit>, since: <stringlit>)^
\begin{quote} Marks a definition as deprecated since the specified version string.
  Accesses to the  annotated entity will then cause a deprecated warning mentioning
  the  message \code{<stringlit>} to be issued from the compiler.
  Deprecated warnings are suppressed in code that belongs itself to a definition
  that is labeled deprecated.
\end{quote}

\lstinline^@deprecatedName(name: <symbollit>)^
\begin{quote} Marks a formal parameter name as deprecated. Invocations of this entity
  using named parameter syntax refering to the deprecated parameter name cause a deprecation warning.
\end{quote}

\section{Scala Compiler Annotations}
