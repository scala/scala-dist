...
Para comeÅÁar, apresentamos um primeiro exemplo, a implementaÅÁÅ„o de Quicksort em Scala.
...
A implementaÅÁÅ„o se parece muito com a que vocÅÍ faria em Java
ou C. NÅÛs usamos os mesmos operadores e estruturas de controle.
Existem tambÅÈm algumas pequenas diferenÅÁas sintÅ·ticas, particularmente:
\begin{itemize}
\item
As declaraÅÁÅıes comeÅÁam usando palavras reservadas. Particularmente, declaraÅÁÅıes de funÅÁÅıes sÅ„o iniciadas
com a palavra \code{def}, declaraÅÁÅıes de variÅ·veis sÅ„o iniciadas com a palavra \code{var} e
declaraÅÁÅ„o de constantes (chamadas de valores) sÅ„o iniciadas com a palavra \code{val}.
\item
O tipo de um parÅ‚metro em uma funÅÁÅ„o ÅÈ declarado apÅÛs o nome do parÅ‚metro seguido de dois pontos(:).
O tipo pode ser omitido quando o compilador for capaz de inferi-lo pelo contexto.
\item
A  declaraÅÁÅ„o de vetores do tipo \code{T} ÅÈ feita usando a expressÅ„o \code{Array[T]} ao invÅÈs de \code{T[]}. 
O i-ÅÈsimo elemento de um vetor \code{a} ÅÈ acessado usando \code{a(i)} ao invÅÈs de \code{a[i]}.
%----------
%\item
...
%parameter to them.
\item
FunÅÁÅıes podem ser aninhadas umas dentro das outras. FunÅÁÅıes aninhadas podem
acessar parÅ‚metros e variÅ·veis locais de suas funÅÁÅıes externas. Por
exemplo, o nome do vetor \code{xs} ÅÈ visÅÌvel nas funÅÁÅıes \code{swap} e 
\code{sort1}, e portanto nÅ„o precisam ser passadas como um parÅ‚metro para elas.
%----------
\end{itemize}
...
com algumas peculiaridades sintÅ·ticas. De fato ÅÈ possÅÌvel escrever
programas em estilo imperativo ou orientado a objetos. Isto ÅÈ 
importante, porque ÅÈ uma das coisas que facilitam combinar componentes
Scala com componentes escritos em linguagens convencionais, tais como Java,
C\# ou Visual Basic.
Entretanto, tambÅÈm ÅÈ possÅÌvel escrever programas num estilo completamente
diferente. Aqui estÅ· o Quicksort novamente, desta vez escrito em 
estilo funcional.
%----------
\begin{lstlisting}
...
%% \end{itemize}
O programa funcional captura a essÅÍncia do algoritmo quicksort de modo conciso:
\begin{itemize}
\item Se o vetor estÅ· vazio ou consiste de um Å˙nico elemento, jÅ· estÅ· ordenado,
      entÅ„o retorne imediatamente.
\item Se o vetor nÅ„o estÅ· vazio, escolha um elemento do meio do vetor como pivÅÙ.
\item Particione o vetor em dois subvetores contendo, respectivamente os elementos
 que sÅ„o menores que o elemento pivÅÙ, maiores que, e um terceiro vetor que contÅÈm
 elementos iguais ao pivÅÙ.
\item Ordene os dois primeiros subvetores por uma chamada recursiva da funÅÁÅ„o de 
ordenaÅÁÅ„o.\footnote{Isso nÅ„o ÅÈ exatamente o que o algoritmo imperativo faz; este 
Å˙ltimo particiona o vetor em dois subvetores contendo elementos menores que ou 
maiores ou iguais ao pivÅÙ.}
\item O resultado ÅÈ obtido pela concatenaÅÁÅ„o dos trÅÍs subvetores.
\end{itemize}
%----------
...
Tanto a implementaÅÁÅ„o imperativa quanto a funcional tem mesma complexidade
assintÅÛtica -- $O(N;log(N))$ no caso mÅÈdio e $O(NúÙ§Ê2)$ no pior caso. Mas onde
a implementaÅÁÅ„o imperativa opera localmente, modificando o vetor original,
a implementaÅÁÅ„o funcional retorna um novo vetor ordenado e deixa o vetor
original inalterado. A implementaÅÁÅ„o funcional, portanto, requer mais
memÅÛria transiente que a imperativa.
%----------
...
A implementaÅÁÅ„o funcional faz parecer que Scala ÅÈ uma linguagem
especializada para operaÅÁÅıes funcionais sobre vetores. De fato, nÅ„o ÅÈ;
todas as operaÅÁÅıes usadas no exemplo sÅ„o simples mÅÈtodos de biblioteca
de uma classe {\em sequÅÍncia} \code{Seq[T]} que ÅÈ parte da biblioteca
padrÅ„o Scala, e onde ela mesma ÅÈ implementada em Scala. Como vetores sÅ„o 
instÅ‚ncias de \verb@Seq@, todos os mÅÈtodos de sequÅÍncia estÅ„o disponÅÌveis
para eles.
%----------
...
Em particular, hÅ· o mÅÈtodo \code{filter} que recebe como argumento uma
{\em funÅÁÅ„o predicado}. Esta funÅÁÅ„o predicado deve mapear elementos do 
vetor para valores boleanos. O resultado de \code{filter} ÅÈ um vetor
consistindo de todos os elementos do vetor original para o qual a funÅÁÅ„o
predicado ÅÈ verdadeira. O mÅÈtodo \code{filter} de um objeto tipo
...
Aqui, \code{T=>Boolean} ÅÈ o tipo das funÅÁÅıes que recebem um elemento 
do tipo \code{T} e retornam um valor booleano do tipo \code{Boolean}. 
FunÅÁÅıes como \code{filter} 
que recebem uma outra funÅÁÅ„o como argumento ou retornam uma funÅÁÅ„o como
resultado sÅ„o chamadas {\em funÅÁÅıes de alta ordem.}     
%----------
...
Scala nÅ„o faz distinÅÁÅ„o entre nomes de identificadores e nomes de operadores.
um identificador pode ser ou uma sequÅÍncia de letras ou digitos que comeÅÁam
com uma letra, ou podem ser uma sequÅÍncia de caracteres especiais, tais como 
``\code{+}'', ``\code{*}'', ou ``\code{:}''.  Qualquer identificador pode
ser usado como operador infixo em Scala. A operaÅÁÅ„o binÅ·ria $E;op;E'$ ÅÈ sempre
interpretado como a chamada de mÅÈtodo $E.op(E')$. Isso vale tambÅÈm para 
operadores binÅ·rios infixos que iniciam com uma letra. Consequentemente,
a expressÅ„o  ~\lstinline@xs filter (pivot >)@~ ÅÈ equivalente Å‡ chamada de
mÅÈtodo ~\lstinline@xs.filter(pivot >)@.
%----------
...
%% \code{pivot}.
No programa quicksort, \code{filter} ÅÈ aplicado trÅÍs vezes a um argumento 
de funÅÁÅ„o anÅÙnima. O primeiro argumento, \code{pivot >}, representa uma 
funÅÁÅ„o que recebe um argumento $x$ e retorna o valor ~\lstinline@pivot > $x$@.
Este ÅÈ um exemplo de uma {\em funÅÁÅ„o parcialmente aplicada}. Um outro modo, 
equivalente de se escrever esta funÅÁÅ„o, que torna o argumento oculto explicito
ÅÈ ~\lstinline@x => pivot > x@. A funÅÁÅ„o ÅÈ anÅÙnima, ou seja, nÅ„o ÅÈ definida
com um nome. O tipo do parÅ‚metro  \code{x} ÅÈ omitido porque um compilador Scala 
pode inferÅÌ-lo automÅ·ticamente a partir do contexto onde a funÅÁÅ„o ÅÈ usada.
Resumindo, \code{xs.filter(pivot >)} retorna uma lista consistindo de todos os 
elementos da lista \code{xs} que sÅ„o menores que \code{pivot}.  
%----------
...
Olhando novamente em detalhes para a primeira, implementaÅÁÅ„o imperativa
do Quicksort, percebemos que muitos dos construtores da linguagem 
usados na segunda soluÅÁÅ„o estÅ„o presentes, embora de modo distinto.
%----------
...
Por exemplo, operadores binÅ·rios padrÅ„o, tais como \code{+}, \code{-},
ou \code{<} nÅ„o sÅ„o tratados de modo especial. Assim como \code{append}, 
sÅ„o mÅÈtodos de seus operandos esquerdos. Consequentemente, a expressÅ„o 
\code{i+1} ÅÈ vista como a invocaÅÁÅ„o de \code{i.+(1)} do mÅÈtodo \code{+}      
do valor inteiro de \code{i}. De fato, um compilador ÅÈ livre (se 
moderadamente esperto, ainda que  esperado) para reconhecer o caso 
especial da chamada de mÅÈtodo \code{+} sobre argumentos inteiros e 
para gerar cÅÛdigo inline eficiente para isso.  
%----------
...
Por eficiÅÍncia e melhor detecÅÁÅ„o de erros o laÅÁo \code{while} ÅÈ um construtor 
primitivo em Scala. Mas em princÅÌpio, poderia do mesmo modo ser uma funÅÁÅ„o 
predefinida. Aqui estÅ· uma possÅÌvel implementaÅÁÅ„o para ele: 
%----------
\begin{lstlisting}
...
A funÅÁÅ„o \code{While} recebe como primeiro parÅ‚metro uma funÅÁÅ„o teste,
que nÅ„o recebe parÅ‚metros e produz um valor boleano. Como segundo parÅ‚metro 
recebe uma funÅÁÅ„o comando que tambÅÈm nÅ„o recebe parÅ‚metros e produz como 
resultado o tipo  \lstinline@Unit@. \code{While} invoca a funÅÁÅ„o comando
enquanto a funÅÁÅ„o teste produzir verdadeiro.
%----------
...
no Java; ÅÈ usado sempre que uma funÅÁÅ„o nÅ„o retornar um resultado interessante.
De fato, porque Scala ÅÈ uma linguagem orientada a expressÅıes, cada funÅÁÅ„o 
retorna algum resultado. Se nenhuma expressÅ„o de retorno ÅÈ  explicitamente
fornecida, o valor \lstinline@()@, que ÅÈ pronunciado ``unit'', ÅÈ assumido.
Este valor ÅÈ do tipo \lstinline@Unit@. FunÅÁÅıes que retornam ``unit'' sÅ„o 
tambÅÈm chamadas {\em procedimentos}. Aqui estÅ· uma formulaÅÁÅ„o mais 
``orientada a expressÅ„o'' da funÅÁÅ„o \lstinline@swap@ na primeira implementaÅÁÅ„o
...
O valor resultante desta funÅÁÅ„o ÅÈ simplesmente sua Å˙ltima expressÅ„o---uma
palavra chave \lstinline@return@ nÅ„o ÅÈ necessÅ·ria. Observe que funÅÁÅıes 
que retornam um valor explÅÌcito sempre precisam de um ``='' antes de
seus corpos ou expressÅıes de definiÅÁÅ„o.
%----------
...
Aqui estÅ· um exemplo que mostra uma Å·rea de aplicaÅÁÅ„o para a qual Scala 
ÅÈ particularmente indicada. Considere a tarefa de implementar um serviÅÁo
de leilÅ„o eletrÅÙnico. Podemos usar um modelo de processo no estilo 
actor do Erlang para implementar os participantes do leilÅ„o. Actors sÅ„o 
objetos para os quais as mensagens sÅ„o enviadas. Cada actor tem uma caixa de correio 
para suas mensagens de entrada que ÅÈ representada para uma fila. Pode
trabalhar sequencialmente nas mensagens da sua caixa de correio, ou buscar 
por mensagens que casam com algum padrÅ„o. 
%----------
...
%% here.
Para cada item negociado hÅ· um actor leiloeiro que publica a 
informaÅÁÅ„o sobre o item negociado, que aceita ofertas de clientes
e que se comunica com o vendedor e com o vencedor do leilÅ„o 
para fechar a transaÅÁÅ„o. Apresentamos uma visÅ„o superficial de uma
 implementaÅÁÅ„o aqui.
%----------
...
Como primeiro passo, definimos as mensagens que sÅ„o trocadas durante um
leilÅ„o. HÅ· duas classes base abstratas \code{AuctionMessage} para mensagens 
de clientes do serviÅÁo de leilÅ„o, e \code{AuctionReply} para respostas 
do serviÅÁo aos clientes. Para ambas as classes base hÅ· um nÅ˙mero de casos 
...
Para cada classe base, hÅ· um nÅ˙mero de {\em classes case} que definem
o formato de mensagens particulares dentro da classe. Estas mensagens
podem em Å˙ltimo caso ser mapeadas a pequenos documentos XML. Esperamos
que hajam ferramentas automÅ·ticas que convertam entre documentos XML e 
estruturas de dados internas, tais como as definidas acima.
% Vinicius - Parei aqui. -> Basile, porque voce nao traduzi Ator?? 
%
% Resp: por que trata-se de um modelo de concorrÅÍncia (o nome do modelo) 
...
A Figura~\ref{fig:simple-auction} apresenta uma implementaÅÁÅ„o Scala para a classe
\code{Auction} para actors do leilÅ„o que coordenam os lances sobre um item. Objetos
para esta classe sÅ„o criados pela indicaÅÁÅ„o
\begin{itemize}
\item Um actor vendedor que precisa ser notificado quando o leilÅ„o terminou,
\item o lance mÅÌnimo,
\item a data de quando o leilÅ„o foi fechado.
\end{itemize}  
O comportamento do actor ÅÈ definido por seu mÅÈtodo \code{act}. Este mÅÈtodo seleciona
repetidamente (usando \code{receiveWithin}) uma mensagem e reage a ela, atÅÈ que o 
leilÅ„o seja fechado, o que ÅÈ sinalizado por uma mensagem \code{TIMEOUT}. Antes de 
finalmente parar, permanece ativo para um outro perÅÌodo determinado pela constante 
\code{timeToShutdown} e replica para ofertas posteriores que o leilÅ„o estÅ· fechado.    
%----------
...
Aqui estÅ„o algumas explicaÅÁÅıes extras sobre os construtores usados neste programa:
\begin{itemize}
\item
O mÅÈtodo \code{receiveWithin} da classe \code{Actor} recebe como parÅ‚metro um prazo dado
em milisegundos e uma funÅÁÅ„o que processa mensagens na caixa de correio. A funÅÁÅ„o ÅÈ dada
por uma sequÅÍncia de cases que especificam um padrÅ„o e uma aÅÁÅ„o para mensagens que 
casam com o padrÅ„o. O mÅÈtodo \code{receiveWithin} seleciona a primeira mensagem da 
caixa de correio que casa com um destes padrÅıes e aplica a aÅÁÅ„o correspondente a ele. 
%----------
...
\item
O Å˙ltimo case de \code{receiveWithin} ÅÈ guardado por um padrÅ„o \code{TIMEOUT}.  
Se nenhuma outra mensagem foi recebida nesse meio tempo, este padrÅ„o ÅÈ disparado
apÅÛs o prazo que foi passado como argumento para o mÅÈtodo envolvente \code{receiveWithin}.
\code{TIMEOUT} ÅÈ uma mensagem especial, que ÅÈ disparada pela prÅÛpria implementaÅÁÅ„o do 
\code{Actor}.    
%----------
...
%% \end{itemize}
\item
Mensagens de resposta sÅ„o enviadas usando sintaxe da forma \code{destino ! AlgumaMensagem}.
\code{!} ÅÈ usado aqui como um operador binÅ·rio com um actor e uma mensagem como argumentos.
Isto ÅÈ equivalente em Scala ao chamado de mÅÈtodo \code{destino.!(AlgumaMensagem)}, ou seja, 
a invocaÅÁÅ„o do mÅÈtodo \code{!} do actor destino com a mensagem dada como parÅ‚metro.
\end{itemize}    
%----------
...
A discussÅ„o precedente deu uma idÅÈia de programaÅÁÅ„o distribuÅÌda em Scala. Isso
dÅ· a sensaÅÁÅ„o que Scala tem um rico conjunto de construtores que suportam
processos actor, envio e recebimento de mensagens, programaÅÁÅ„o com timeouts etc.
De fato, o oposto ÅÈ verdadeiro. Todos os construtores discutidos acima sÅ„o 
oferecidos como mÅÈtodos na classe biblioteca \code{Actor}. Aquela classe ÅÈ
ele mesma implementada em Scala, baseado no modelo thread subjacente Å‡ linguagem 
hospedeira (Java ou .NET). A implementaÅÁÅ„o de todas as caracterÅÌsticas da 
classe \code{Actor} usada aqui ÅÈ dada na SeÅÁÅ„o~\ref{sec:actors}.     
%----------
...
As vantagens da abordagem baseada em biblioteca sÅ„o a relativa simplicidade
da linguagem nÅ˙cleo e a flexibilidade para os criadores de biblioteca. Como 
a linguagem nÅ˙cleo nÅ„o precisa especificar detalhes da comunicaÅÁÅ„o dos 
processos de alto nÅÌvel, pode ser mantida mais simples e geral. Como
um modelo particular de mensagens numa caixa de correio ÅÈ um mÅÛdulo biblioteca, 
pode ser modificado livremente se um diferente modelo for necessÅ·rio em algumas 
aplicaÅÁÅıes. A abordagem requer, entretanto, que a linguagem nÅ˙cleo seja expressiva
o suficiente para prover as abstraÅÁÅıes linguÅÌsticas necessÅ·rias de um modo 
conveniente. Scala foi criada com isto em mente; um de seus maiores objetivos de 
design foi deixÅ·-la flexÅÌvel o suficiente para atuar como uma conveniente linguagem
hospedeira para domÅÌnios de linguagens especÅÌficos implementados por mÅÛdulos de 
biblioteca. Por exemplo, a construÅÁÅ„o de comunicaÅÁÅ„o actor apresentada acima
pode ser vista como um desses domÅÌnios especÅÌficos de linguagem, que conceitualmente
estendem o nÅ˙cleo Scala.  
%----------

%----------
% Vinicius - comecei traducao aqui.
%%\chapter{\label{chap:simple-funs}Expressions and Simple Functions}
\chapter{\label{chap:simple-funs}ExpressÅıes e FunÅÁÅıes Simples}
...
introduzimos as suas construÅÁÅıes de linguagem uma a uma de uma maneira mais sistemÅ·tica.
Vamos comeÅÁar com os menores elementos: expressÅıes e funÅÁÅıes.


%%\section{Expressions And Simple Functions}
\section{ExpressÅıes e FunÅÁÅıes Simples}
...
O usuÅ·rio interage com a calculadora digitando expressÅıes. A calculadora retorna o
resultado do cÅ·lculo e o seu tipo de dado. Por exemplo: 
...
TambÅÈm ÅÈ possÅÌvel dar nome a uma sub-expressÅ„o e usar o nome, ao invÅÈs da expressÅ„o, 
nas expressÅıes seguintes:
\begin{lstlisting}
scala> def scale = 5
scale: Int
...
DefiniÅÁÅıes comeÅÁam com a palavra reservada \code{def}. Elas introduzem um nome
que representa a expressÅ„o que vem depois do sÅÌmbolo \code{=}. O intepretrador
...
Executando uma definiÅÁÅ„o tal como \code{def x = e} nÅ„o avaliarÅ· a expressÅ„o \code{e}. 
Ao invÅÈs \code{e} ÅÈ avaliado sempre que \code{x} for usado. Alternativamente, Scala 
oferece um valor definiÅÁÅ„o \code{val x = e}, o qual avalia o lado direito de \code{e}
como parte da avaliaÅÁÅ„o da definiÅÁÅ„o. Se \code{x} ÅÈ entÅ„o usado subsequentemente, ÅÈ 
imediatamente substituÅÌdo pelo valor prÅÈ-computado de \code{e}, logo a expressÅ„o nÅ„o 
...
%% or a list.
Como as expressÅıes sÅ„o avaliadas? Uma expressÅ„o consistindo de operadores e 
operandos ÅÈ avaliada pela repetida aplicaÅÁÅ„o dos seguintes passos de simplificaÅÁÅ„o:
\begin{itemize}
\item escolha a operaÅÁÅ„o mais a esquerda.
\item avalie seu operando
\item aplique o operador aos valores do operando.
\end{itemize}
Um nome definido por \code{def} ÅÈ avaliado substituindo o nome pela definiÅÁÅ„o do lado
direito (nÅ„o avaliada). Um nome definido por \code{val} ÅÈ avaliado pela substituiÅÁÅ„o do 
nome pelo valor da definiÅÁÅ„o do lado direito. O processo de avaliaÅÁÅ„o pÅ·ra assim que 
encontrarmos um valor. Um valor ÅÈ algum dado, tal como uma cadeia de caracteres, um nÅ˙mero, 
...
\example
Aqui estÅ· uma avaliaÅÁÅ„o de uma expressÅ„o aritmÅÈtica.
...
$\rightarrow$  62.83185307179586
\end{lstlisting}
O processo de simplificar gradualmente expressÅıes para valores ÅÈ 
chamado {\em reduÅÁÅ„o}.
%----------
...
Usando \code{def}, pode-se tambÅÈm definir funÅÁÅıes com parÅ‚metros. Por exemplo:
\begin{lstlisting}
...
ParÅ‚metros de funÅÁÅıes seguem o nome da funÅÁÅ„o e sempre sÅ„o envolvidos por
parÅÍnteses. Cada parÅ‚metro vem com um tipo que segue o nome do parÅ‚metro
e dois pontos. AtÅÈ aqui, sÅÛ precisamos de tipos numÅÈricos, tal como o tipo
\code{scala.Double} dos nÅ˙meros de precisÅ„o dupla. Scala define {\em tipos
aliases} para alguns tipos bÅ·sicos, logo podemos escrever tipos numÅÈricos 
como em Java. Por exemplo, \code{double} ÅÈ um tipo alias de \code{scala.Double}
e \code{int} ÅÈ um tipo alias para \code{scala.Int}.       
%----------
...
%% arguments.
FunÅÁÅıes com parÅ‚metros sÅ„o avaliadas analogamente a operadores em expressÅıes.
Primeiro, os argumentos da funÅÁÅ„o sÅ„o avaliados (da esquerda para direita). 
EntÅ„o, a aplicaÅÁÅ„o da funÅÁÅ„o ÅÈ substituÅÌdo pelo lado direito da funÅÁÅ„o, e
ao mesmo tempo todos os parÅ‚metros formais sÅ„o substituÅÌdos pelos seus 
argumentos atuais. 
%----------
...
O exemplo mostra que o interpretador reduz argumentos de funÅÁÅıes a valores 
antes de reescrever a aplicaÅÁÅ„o da funÅÁÅ„o. Pode-se ao invÅÈs escolher aplicar
a funÅÁÅ„o a argumentos nÅ„o reduzidos. Isto levarÅ· a seguinte sequÅÍncia de 
reduÅÁÅ„o: 
%----------
...
%% values.  
A segunda ordem de avaliaÅÁÅ„o ÅÈ conhecida como \emph{chamada por nome},
e a primeira por \emph{chamada por valor}. Para expressÅıes que se utilizam
apenas de funÅÁÅıes e que portanto podem ser reduzidas com o modelo de 
substituiÅÁÅ„o, ambos os esquemas levam ao mesmo valor final.
%----------
...
Chamada por valor tem a vantagem de evitar avaliaÅÁÅıes repetidas de argumentos.
Chamada por nome tem a vantagem de evitar avaliaÅÁÅıes de argumentos quando o 
parÅ‚metro nÅ„o ÅÈ usado pela funÅÁÅ„o. Chamada por valor ÅÈ geralmente mais 
eficiente que chamada por nome, mas uma avaliaÅÁÅ„o de chamada por valor
pode entrar em laÅÁo infinito, enquanto uma avaliaÅÁÅ„o de chamada por nome 
termina. Considere: 
%----------
\begin{lstlisting}
...
EntÅ„o \code{first(1, loop)} ÅÈ reduzido com uma chamada por nome a \code{1},
enquanto o mesmo termo, atravÅÈs de uma chamada por valor reduz a si mesmo 
repetidamente, logo a avaliaÅÁÅ„o nÅ„o termina.  
%----------
\begin{lstlisting}
$\,\,\,$   first(1, loop)
...
Scala usa chamada por valor por default, mas muda para avaliaÅÁÅ„o de chamada por nome 
se o tipo do parÅ‚metro for precedido por \code{=>}. 
\example\ 
...
scala> constOne(loop, 2)               // leva a laÅÁo infinito
^C                                     // para a execuÅÁÅ„o com Ctrl-C
\end{lstlisting}
%----------
...
%% expression \code{ ... ? ... : ...}.
\section{ExpressÅıes Condicionais}
O \code{if-else} do Scala leva a uma escolha entre duas alternativas. Sua
sintaxe ÅÈ a mesma do \code{if-else} do Java. Mas onde o \code{if-else} do 
Java pode ser usado somente como uma alternativa entre comandos, Scala 
permite a mesma sintaxe para escolher entre duas expressÅıes. Isso porque o
\code{if-else} do Scala serve tambÅÈm como um substituto para a expressÅ„o 
...
abs: (Double)Double
\end{lstlisting}
ExpressÅıes boleanas em Scala sÅ„o similares as em Java; sÅ„o formadas
a partir de constantes.  
\code{true} e
\code{false}, operadores de comparaÅÁÅ„o, negaÅÁÅ„o boleana \code{!} e os 
...
%% \section{\label{sec:sqrt}Example: Raiz Quadrada pelo MÅÈtodo de Newton}
Agora ilustraremos elementos da linguagem intruduzidos atÅÈ aqui na
construÅÁÅ„o de um programa mais interessante. A tarefa ÅÈ escrever um funÅÁÅ„o  
 \begin{lstlisting}
 def sqrt(x: Double): Double = ...
 \end{lstlisting}
...
%% $\sqrt 2$.
Um modo comum de se computar raizes quadradas ÅÈ pelo mÅÈtodo das aproximaÅÁÅıes
sucessivas de Newton. Inicia-se com um palpite inicial \code{y} (digamos:
\code{y = 1}). EntÅ„o melhora-se repetidamente o atual palpite \code{y} 
tomando-se a mÅÈdia de \code{y} e \code{x/y}. Como um exemplo, as prÅÛximas
trÅÍs colunas indicam o palpite \code{y}, o quociente \code{x/y}, e suas
mÅÈdias para as primeiras aproximaÅÁÅıes para $\sqrt 2$.       
...
Este algoritmo pode ser implementado em Scala por um conjunto de pequenas funÅÁÅıes,
onde cada uma representa um dos elementos do algoritmo.

Primeiro definimos uma funÅÁÅ„o para iterar do palpite para o resultado:
%-----------
...
Observe que \code{sqrtIter} chama a si mesmo recursivamente. LaÅÁos em 
programas imperativos podem sempre ser modelados por recursÅ„o em 
...
Observe tambÅÈm que a definiÅÁÅ„o de \code{sqrtIter} contÅÈm um tipo retorno,
que segue o seÅÁÅ„o de parÅ‚metros. Tais tipos de retorno sÅ„o mandatÅÛrios para 
funÅÁÅıes recursivas. Para uma funÅÁÅ„o nÅ„o recursiva, o tipo de retorno ÅÈ opcional;
se estiver faltando o verificador de tipos o computarÅ· a partir do tipo do lado
direito da funÅÁÅ„o. Entretanto, mesmo para funÅÁÅıes nÅ„o recursivas ÅÈ sempre boa idÅÈia 
incluir um tipo de retorno para melhor documentaÅÁÅ„o.  
%----------
...
\section{Aninhamento de FunÅÁÅıes}

A programaÅÁÅ„o funcional encoraja a construÅÁÅ„o de muitas pequenas funÅÁÅıes
auxiliares. Nos Å˙ltimos exemplos, a implementaÅÁÅ„o de \code{sqrt} faz uso da
funÅÁÅıes auxiliares \code{sqrtIter}, \code{improve} e \code{isGoodEnough}.
Os nomes destas funÅÁÅıes sÅ„o relevantes somente para a implementaÅÁÅ„o de 
\code{sqrt}. Normalmente nÅ„o queremos que os usuÅ·rios de \code{sqrt} acessem
estas funÅÁÅıes diretamente.  
%----------

%----------
%% We can enforce this (and avoid name-space pollution) by including
%% the helper functions within the calling function itself:
NÅÛs podemos reforÅÁar isto (e evitar poluiÅÁÅ„o de nomes) incluindo
funÅÁÅıes auxiliares dentro das prÅÛprias funÅÁÅıes: 
\begin{lstlisting}
def sqrt(x: Double) = {
  def sqrtIter(guess: Double, x: Double): Double =
    if (isGoodEnough(guess, x)) guess
...
Blocos em Scala sÅ„o eles mesmos expressÅıes. Cada bloco termina com um
expressÅ„o resultado o qual define seu valor. A expressÅ„o  resultado pode 
ser precedida por definiÅÁÅıes auxiliares, as quais sÅ„o visÅÌveis somente no
prÅÛprio bloco. 
%----------
...
%% true.
Cada definiÅÁÅ„o no bloco pode ser seguida para um ponto e vÅÌrgula, o qual 
separa esta definiÅÁÅ„o das definiÅÁÅıes subsequentes ou a expressÅ„o  resultado.
Entretanto, um ponto e vÅÌrgula ÅÈ inserido implicitamente ao final de cada linha,
a nÅ„o ser que uma das condiÅÁÅıes a seguir seja verdadeira.
%----------
...
\begin{enumerate}
\item 
Ou a linha em questÅ„o termina com uma palavra tal que um ponto ou um 
operador infixo nÅ„o sÅ„o legais ao final da expressÅ„o.
\item 
Ou a prÅÛxima linha inicia com uma palavra que nÅ„o pode iniciar uma expressÅ„o.
\item
Ou estamos dentro de parÅÍnteses \prog{$($...$)$} ou chaves \prog{}, porque 
estes nÅ„o podem conter multiplos comandos de qualquer modo.
\end{enumerate}
%----------

%----------
%% Therefore, the following are all legal:
Entretanto, os seguintes sÅ„o legais:
...
def g2(x: Int) = {x + 1};  /* `;' mandatÅÛrio */ g2(1) + g2(2)
...
def h2(x: Int) = (
  x     // parÅÍnteses mandatÅÛrio, senÅ„o um ponto e vÅÌrgula
  + y   // serÅ· inserido apÅÛs o `x'.
)
h2(1) / h2(2)
\end{lstlisting}
%----------
...
algum outro bloco ÅÈ tambÅÈm visÅÌvel em algum bloco interno, desde que nÅ„o tenha
sido redefinido lÅ·. Esta regra nos permite simplificar nosso exemplo \code{sqrt}.
NÅ„o precisamos passar \code{x} como parÅ‚metro adicional de funÅÁÅıes aninhadas, dado
que estÅ· sempre visÅÌvel nelas como um parÅ‚metro da funÅÁÅ„o externa \code{sqrt}.
Aqui estÅ· o cÅÛdigo simplificado:
...
Considere a seguinte funÅÁÅ„o para calcular o maior divisor comum entre
dois nÅ˙meros dados.
...
Usando nosso modelo de substituiÅÁÅ„o da funÅÁÅ„o de avaliaÅÁÅ„o,
...
Contraste isto com a avaliaÅÁÅ„o de uma outra funÅÁÅ„o recursiva,
\code{factorial}:
...
A aplicaÅÁÅ„o de \code{factorial(5)} ÅÈ reescrita como segue: 
...
HÅ· uma importante diferenÅÁa entre as duas sentenÅÁas reescritas:
Os termos na sequÅÍncia reescrita de \code{gcd} tem repetidamente a
mesma forma. Conforme a avaliaÅÁÅ„o prossegue, seu tamanho ÅÈ limitado por
uma constante. Diferentemente, na avaliaÅÁÅ„o do fatorial obtemos cadeias
cada vez mais longas de operandos que sÅ„o entÅ„o multiplicados na Å˙ltima
parte da sequÅÍncia de avaliaÅÁÅ„o. 
%----------
...
Ainda que as atuais implementaÅÁÅıes de Scala nÅ„o trabalhem reescrevendo
termos, elas contudo devem ter o mesmo comportamento de espaÅÁo que nas
sequÅÍncias reescritas. Na implementaÅÁÅ„o de \code{gcd}, nota-se que  a 
chamada recursiva para \code{gcd} ÅÈ a Å˙ltima aÅÁÅ„o realizada na avaliaÅÁÅ„o 
do seu corpo. Pode-se tambÅÈm dizer que \code{gcd} ÅÈ uma recursÅ„o de cauda
(tail-recursive). A Å˙ltima chamada numa funÅÁÅ„o recursiva de cauda pode ser
implementada por um salto ao inÅÌcio da funÅÁÅ„o. Os argumentos dessa chamada 
pode sobrescrever os parÅ‚metros da atual instanciaÅÁÅ„o de \code{gcd}, portanto 
nenhum espaÅÁo na pilha ÅÈ necessÅ·rio. Consequentemente, funÅÁÅıes recursivas de 
cauda sÅ„o processos iterativos, que podem ser executados em espaÅÁo constante.     
%----------
...
Em contraste, a chamada recursiva em \code{factorial} ÅÈ seguida por uma 
multiplicaÅÁÅ„o. consequentemente, um novo espaÅÁo na pilha ÅÈ alocado para a 
instÅ‚ncia recursiva de fatorial, e ÅÈ desalocada apÅÛs o tÅÈrmino daquela 
instÅ‚ncia. A formulaÅÁÅ„o dada para a funÅÁÅ„o fatorial nÅ„o ÅÈ recursiva de cauda;
 precisa de espaÅÁo proporcional aos seus parÅ‚metros de entrada para sua
execuÅÁÅ„o. 
%----------
...
Genericamente, se a Å˙ltima aÅÁÅ„o da funÅÁÅ„o ÅÈ uma chamada a outra funÅÁÅ„o
(possivelmente a mesma), apenas um Å˙nico espaÅÁo na pilha ÅÈ requerido para 
ambas funÅÁÅıes. Tais chamadas sÅ„o denominadas ``tail call''. Em 
princÅÌpio, tail calls sempre podem reutilizar o espaÅÁo da pilha da funÅÁÅ„o 
de chamada. Entretanto, alguns ambientes de execuÅÁÅ„o (tais como Java VM) 
carecem das primitivas para fazer um espaÅÁo de pilha reutilizÅ·vel para 
tail calls eficientes. A produÅÁÅ„o de uma implementaÅÁÅ„o Scala de qualidade ÅÈ,
portanto, requerida somente para reutilizar o espaÅÁo de pilha de uma funÅÁÅ„o 
recursiva de cauda cuja Å˙ltima aÅÁÅ„o ÅÈ chamar a si mesma. Outras chamadas de 
cauda podem ser otimizadas tambÅÈm, mas nÅ„o deve-se confiar nisso ao longo das
implementaÅÁÅıes. 
%----------

\begin{exercise} Escreva uma implementaÅÁÅ„o recursiva de cauda de 
\code{factorial}.
\end{exercise}
...
%% for program composition.
 ...
 \chapter{\label{chap:first-class-funs}FunÅÁÅıes de Primeira Classe}
Uma funÅÁÅ„o em Scala ÅÈ um valor de primeira classe. Como qualquer outro valor, 
pode ser passado como um parÅ‚metro ou retornado como um resultado. FunÅÁÅıes que 
recebem outras funÅÁÅıes como parÅ‚metros ou as retornam como resultados sÅ„o 
denominadas funÅÁÅıes de {\em alta ordem}. Este capÅÌtulo introduz funÅÁÅıes de alta ordem 
e mostra como elas fornecem um mecanismo flexÅÌvel para a composiÅÁÅ„o de programas.
%----------

%----------
%% As a motivating example, consider the following three related tasks:
Como um exemplo de motivaÅÁÅ„o, considere as trÅÍs tarefas relacionadas a seguir:
\begin{enumerate}
\item
%%Write a function to sum all integers between two given numbers \code{a} and \code{b}:
Escreva uma funÅÁÅ„o que some todos os inteiros entre dois dados nÅ˙meros, \code{a} e \code{b}:  
\begin{lstlisting}
...
%% \code{a} and \code{b}:
Escreva uma funÅÁÅ„o para somar os quadrados de todos os inteiros entre dois dados nÅ˙meros, 
 \code{a} e \code{b}:
...
Escreva uma funÅÁÅ„o para somar as potÅÍncias $2^n$ de todos os $n$ inteiros entre
dois dados nÅ˙meros \code{a} e \code{b}:  
...
Estas funÅÁÅıes sÅ„o todas instÅ‚ncias de \(\sum^b_a f(n)\) para diferentes valores de $x$.
Podemos obter o padrÅ„o comum definindo uma funÅÁÅ„o \code{sum}: 
\begin{lstlisting}
...
%% function.
O tipo \code{Int => Int} ÅÈ o tipo de funÅÁÅıes que recebem argumentos do tipo
\code{Int} e retornam resultados do tipo \code{Int}. EntÅ„o \code{sum} ÅÈ uma 
funÅÁÅ„o que recebe uma outra funÅÁÅ„o como parÅ‚metro. Em outras palavras, \code{sum}
ÅÈ uma funÅÁÅ„o de {\em alta ordem}.     
%----------

%----------
%% Using \code{sum}, we can formulate the three summing functions as
%% follows.
Usando \code{sum}, podemos formular as trÅÍs funÅÁÅıes de soma como segue: 
\begin{lstlisting}
...
\section{FunÅÁÅıes AnÅÙnimas}
ParametrizaÅÁÅ„o por funÅÁÅıes tende a criar vÅ·rias pequenas funÅÁÅıes. No exemplo 
anterior, definimos \code{id}, \code{square} e \code{power} como funÅÁÅıes 
...
Ao invÅÈs de usarmos definiÅÁÅıes de funÅÁÅıes nomeadas para estas pequenas funÅÁÅıes 
argumentos, podemos formulÅ·-las de um modo abreviado como {\em funÅÁÅıes anÅÙnimas}.
Uma funÅÁÅ„o anÅÙnima ÅÈ uma expressÅ„o que ÅÈ avaliada para uma funÅÁÅ„o; a funÅÁÅ„o ÅÈ 
definida sem receber um nome. Como exemplo considere a funÅÁÅ„o anÅÙnima quadrado:
\begin{lstlisting}
  (x: Int) => x * x
\end{lstlisting}
%----------
...
A parte anterior a flecha `\code{=>}' sÅ„o os parÅ‚metros da funÅÁÅ„o, ao passo que a parte
seguinte a `\code{=>}' ÅÈ o seu corpo.
Por exemplo, aqui estÅ· uma funÅÁÅ„o anÅÙnima que multiplica seus dois argumentos.
\begin{lstlisting}
  (x: Int, y: Int) => x * y
\end{lstlisting}
%% Using anonymous functions, we can reformulate the first two summation
%% functions without named auxiliary functions:
Usando funÅÁÅıes anÅÙnimas, podemos reformular as duas funÅÁÅıes soma sem
funÅÁÅıes auxiliares nomeadas:
\begin{lstlisting}
...
Frequentemente, o compilador Scala pode deduzir o tipo do parÅ‚metro a partir
do contexto da funÅÁÅ„o anÅÙnima nos casos em que foram omitidos.
Por exemplo, no caso de \code{sumInts} ou \code{sumSquares}, sabe-se a partir do 
tipo de \code{sum} que o primeiro parÅ‚metro deve ser uma funÅÁÅ„o de tipo \code{Int => Int}.
Consequentemente, o tipo do parÅ‚metro \code{Int} ÅÈ redundante e pode ser omitido. Se
houver um Å˙nico parÅ‚metro sem um tipo, tambÅÈm podemos omitir os parÅ‚metros a sua volta. 
...
Em geral, o termo em Scala 
 \code{(x}$_1$\code{: T}$_1$\code{, ..., x}$_n$\code{: T}$_n$\code{) => E} 
define uma funÅÁÅ„o que mapeia seus parÅ‚metros \code{x}$_1$\code{, ..., x}$_n$
ao resultado da expressÅ„o \code{E} (onde \code{E} pode referir-se a   
\code{x}$_1$\code{, ..., x}$_n$). FunÅÁÅıes anÅÙnimas nÅ„o sÅ„o elementos essenciais
da linguagem Scala, dado que sempre podem ser expressos em termos de funÅÁÅıes
nomeadas. De fato, a funÅÁÅ„o anÅÙnima 

\begin{lstlisting}
(x$_1$: T$_1$, ..., x$_n$: T$_n$) => E
\end{lstlisting}
%% is equivalent to the block
ÅÈ equivalente ao bloco
\begin{lstlisting}
...
onde \code{f} ÅÈ um novo nome que nÅ„o ÅÈ utilizado em qualquer outro lugar do programa.
TambÅÈm dizemos que funÅÁÅıes anÅÙnimas sÅ„o ``syntactic sugar''. 
%----------
...
A Å˙ltima formulaÅÁÅ„o das funÅÁÅıes de soma jÅ· sÅ„o bem compactas. Mas 
podemos fazer ainda melhor. Observe que \code{a} e \code{b} aparecem como 
parÅ‚metros e argumentos de cada funÅÁÅ„o, mas nÅ„o parecem tomar parte de 
combinaÅÁÅıes interessantes. HÅ· algum modo de nos livrarmos delas?

Vamos tentar reescrever \code{sum} de tal modo que nÅ„o receba os limites
\code{a} e \code{b} como parÅ‚metros:     
...
Nessa formulaÅÁÅ„o, \code{sum} ÅÈ uma funÅÁÅ„o que retorna uma outra funÅÁÅ„o, especificamente
a funÅÁÅ„o especializada soma \code{sumF}. Esta Å˙ltima funÅÁÅ„o realiza todo o trabalho; 
recebe os limites \code{a} e \code{b} como parÅ‚metros, aplica a funÅÁÅ„o \code{f}, parÅ‚metro
de \code{sum}, a todos os inteiros entre eles, e soma os resultados.
      
%% Using this new formulation of \code{sum}, we can now define:
Usando esta nova formulaÅÁÅ„o de \code{sum}, podemos agora definir:
\begin{lstlisting}
def sumInts  =  sum(x => x)
...
Ou, equivalentemente, com definiÅÁÅıes de valores:
\begin{lstlisting}
val sumInts  =  sum(x => x)
...
aplicados como qualquer outra funÅÁÅ„o. Por exemplo,
\begin{lstlisting}
...
Como funÅÁÅıes que retornam funÅÁÅıes sÅ„o aplicadas? Como exemplo, na expressÅ„o 
\begin{lstlisting}
sum(x => x * x)(1, 10) ,
\end{lstlisting}
...
a funÅÁÅ„o \code{sum} ÅÈ aplicada a funÅÁÅ„o quadrada \code{(x => x * x)}. 
O funÅÁÅ„o resultante ÅÈ entÅ„o aplicada ao segunda lista de argumentos, \code{(1, 10)}.   
%----------
...
%% \code{(sum(x => x * x))(1, 10)}.
Essa notaÅÁÅ„o ÅÈ possÅÌvel porque aplicaÅÁÅıes de funÅÁÅıes sÅ„o associativas Å‡ esquerda.
Ou seja, se $\mbox{args}_1$ e $\mbox{args}_2$ sÅ„o listas de argumentos, entÅ„o 
\bda{lcl}
f(\mbox{args}_1)(\mbox{args}_2) & \ \ \mbox{ÅÈ equivalente a}\ \ & (f(\mbox{args}_1))(\mbox{args}_2)
\eda
Em nosso exemplo, \code{sum(x => x * x)(1, 10)} ÅÈ equivalente a seguinte expressÅ„o:
\code{(sum(x => x * x))(1, 10)}.
%----------
...
O estilo ``funÅÁÅıes que retornam funÅÁÅıes'' ÅÈ tÅ„o Å˙til que Scala tem sintaxe
especial para ele. Por exemplo, a prÅÛxima definiÅÁÅ„o de \code{sum} ÅÈ equivalente
a anterior, porÅÈm mais curta: 
\begin{lstlisting}
def sum(f: Int => Int)(a: Int, b: Int): Int =
  if (a > b) 0 else f(a) + sum(f)(a + 1, b)
\end{lstlisting}
%% Generally, a curried function definition 
Genericamente, uma definiÅÁÅ„o de funÅÁÅ„o currificada
\begin{lstlisting}
...
onde \code{g} ÅÈ um novo identificador. Ou, menor, usando uma funÅÁÅ„o anÅÙnima: 
\begin{lstlisting}
def f (args$_1$) ... (args$_{n-1}$) = ( args$_n$ ) => E .
\end{lstlisting}
%%Performing this step $n$ times yields that
Realizando este passo $n$ vezes dÅ· aquela 
\begin{lstlisting}
def f (args$_1$) ... (args$_n$) = E
\end{lstlisting}
%%is equivalent to
...
Ou, equivalentemente, usando uma definiÅÁÅ„o valor:
\begin{lstlisting}
...
Este estilo de definiÅÁÅ„o de funÅÁÅ„o e aplicaÅÁÅ„o ÅÈ chamado {\em currificado}
depois que seu divulgador, Haskell B.\ Curry, um lÅÛgico do sÅÈculo $20$, mesmo 
que a idÅÈia nos leve de volta a Moses Sch\"onfinkel e Gottlob Frege.
%----------
...
O tipo de uma funÅÁÅ„o que retorna funÅÁÅ„o ÅÈ expresso de modo anÅ·logo a sua lista
de parÅ‚metros. Tomando a Å˙ltima formulaÅÁÅ„o de \code{sum} como exemplo, o tipo 
para \code{sum} ÅÈ \code{(Int => Int) => (Int, Int) => Int}.
Isso ÅÈ possÅÌvel porque tipos de funÅÁÅıes sÅ„o associativos Å‡ direita. Ou seja, 
\begin{lstlisting}
...
1. A funÅÁÅ„o \code{sum} usa uma recursiva linear. VocÅÍ poderia alterÅ·-la
para uma funÅÁÅ„o recursiva de cauda, preenchendo as ?? abaixo?  
\begin{lstlisting}
def sum(f: Int => Int)(a: Int, b: Int): Int = {
...
Escreva uma funÅÁÅ„o \code{produto} que computa o produto de valores de
funÅÁÅıes em pontos sobre um dado intervalo.  
\end{exercise}
...
Escreva uma funÅÁÅ„o ainda mais genÅÈrica que generaliza ambos \code{sum} e \code{produto}.  
\end{exercise}
%----------
...
\section{Exemplo: Encontrando Pontos Fixos de FunÅÁÅıes}

Um nÅ˙mero \code{x} ÅÈ chamado um {\em ponto fixo} de uma funÅÁÅ„o \code{f} se  
\begin{lstlisting}
f(x) = x .
\end{lstlisting}
...
Para algumas funÅÁÅıes \code{f} podemos encontrar os pontos fixos iniciando
com um palpite inicial e entÅ„o aplicando \code{f} repetidamente, atÅÈ que o valor 
nÅ„o mude mais (ou a mudanÅÁa seja tolerÅ·vel). Isso ÅÈ possÅÌvel na sequÅÍncia  
\begin{lstlisting}
x, f(x), f(f(x)), f(f(f(x))), ...
\end{lstlisting}
%%converges to fixed point of $f$. This idea is captured in
%%the following ``fixed-point finding function'':
converge para pontos fixos de $f$. Essa idÅÈia ÅÈ captada na ``funÅÁÅ„o achadora 
...
Agora aplicaremos esta idÅÈia na reformulaÅÁÅ„o da funÅÁÅ„o raiz quadrada.
Vamos comeÅÁar pela especificaÅÁÅ„o de \code{sqrt}:  
\begin{lstlisting}
...
Consequentemente, \code{sqrt(x)} ÅÈ um ponto fixo da funÅÁÅ„o \code{y => x / y}.
Isso sugere que \code{sqrt(x)} pode ser computado por uma iteraÅÁÅ„o de ponto fixo: 
\begin{lstlisting}
...
Mas se tentarmos isso, percebemos que aquela computaÅÁÅ„o nÅ„o converge. 
Vamos agregar Å‡ funÅÁÅ„o de ponto fixo um comando print que mostra o 
...
Um modo de controlar tais oscilaÅÁÅıes ÅÈ prevenir guess de mudar muito.
Isso pode ser obtido pela mÅÈdia (funÅÁÅ„o {\em averaging}) de sucessivos valores da sequÅÍncia 
original:
\begin{lstlisting}
...
De fato, expandindo a funÅÁÅ„o \code{fixedPoint} dÅ· exatamente nossa prÅÈvia
definiÅÁÅ„o de ponto fixo da SeÅÁÅ„o~\ref{sec:sqrt}.
...
ÅÈ consideravelmente aumentado se funÅÁÅıes puderem ser passadas como argumentos.
O prÅÛximo exemplo mostra que funÅÁÅıes que retornam funÅÁÅıes tambÅÈm podem ser
muito Å˙teis.
%----------
...
Considere novamente iteraÅÁÅıes de ponto fixo. ComeÅÁamos observando que 
$\sqrt(x)$ ÅÈ um ponto fixo da funÅÁÅ„o \code{y => x / y}. EntÅ„o fazemos
a iteraÅÁÅ„o convergir tirando a mÅÈdia de sucessivos valores. Essa tÅÈcnica
de {\em amortecimento da mÅÈdia} ÅÈ tÅ„o genÅÈrica que pode ser denotada
em outra funÅÁÅ„o.
\begin{lstlisting}
...
%% as follows.
Usando \code{averageDamp}, podemos reformular a funÅÁÅ„o da raiz quadrada
como segue. 
\begin{lstlisting}
def sqrt(x: Double) = fixedPoint(averageDamp(y => x/y))(1.0)
\end{lstlisting}
%% This expresses the elements of the algorithm as clearly as possible.
Isto expressa os elementos do algoritmo tÅ„o claramente quanto possÅÌvel.
\begin{exercise} 
%%Write a function for cube roots using \code{fixedPoint} and 
%% \code{averageDamp}.
Escreva uma funÅÁÅ„o para a raiz cÅ˙bica usando \code{fixedPoint} e \code{averageDamp}.  
\end{exercise}
%----------
...
Vimos no capÅÌtulo anterior que funÅÁÅıes sÅ„o abstraÅÁÅıes essenciais porque 
nos permitem introduzir mÅÈtodos genÅÈricos de computaÅÁÅ„o como explÅÌcitos,
elementos nomeados em nossa linguagem de programaÅÁÅ„o. O presente capÅÌtulo
mostrou que tais abstraÅÁÅıes podem ser combinados por funÅÁÅıes de alta ordem
para criar mais abstraÅÁÅıes. Como programadores, devemos procurar por oportunidades 
de reuso e abstraÅÁÅ„o. O mais alto nÅÌvel possÅÌvel de abstraÅÁÅ„o nem sempre ÅÈ 
o melhor, mas ÅÈ importante saber tÅÈcnicas de abstraÅÁÅ„o, de modo que possamos
usÅ·-las quando apropriado. 
%----------

%----------
%% \section{Language Elements Seen So Far}
\section{Elementos da Linguagem Vistos AtÅÈ Aqui}
...
%% (0 or more occurrences).
 Os capÅÌtulos ~\ref{chap:simple-funs} e \ref{chap:first-class-funs} trataram
elementos da linguagem Scala para expressar tipos e expressÅıes relacionados a
tipos primitivos e funÅÁÅıes. A sintaxe livre de contexto desses elementos da
linguagem sÅ„o dados abaixo em formato Backus-Naur estendido, onde `\code{|}'
denotam alternativas, \code{[...]} denotam opÅÁÅ„o  (0 ou 1 ocorrÅÍncia), e  
\lstinline@{...}@ denotam (0 ou mais ocorrÅÍncias). 
%% \subsection*{Characters}
\subsection*{Caracteres}
%% Scala programs are sequences of (Unicode) characters. We distinguish the
%% following character sets:
Programas em Scala sÅ„o sequÅÍncia de caracteres (Unicode). Distinguimos os
seguintes conjuntos de caracteres.
 \begin{itemize}
 \item
%% whitespace, such as `\code{ }', tabulator, or newline characters,
espaÅÁo em branco, tal como `\code{ }', tabulaÅÁÅ„o, ou caracter de mudanÅÁa de linha (newline), 
 \item
...
Essencialmente, hÅ· caracteres imprimÅÌveis que nÅ„o estÅ„o em nenhum dos 
...
Literais sÅ„o como em Java. Eles definem nÅ˙meros, caracteres, cadeias de caracteres, ou 
...
Identificadores podem ter duas formas. Eles ou iniciam por uma letra, que ÅÈ 
seguida por uma sequÅÍncia (possivelmente vazia) de letras ou sÅÌmbolos, ou podem 
iniciar com um caracter operador, seguido por uma sequÅÍncia (possivelmente vazia) de
caracteres operadores. Ambas as formas podem conter caracteres ``underscore'' `\code{_}'.
AlÅÈm disso, um caracter underscore pode ser seguido por quaisquer identificadores.
Consequentemente, todos a seguir sÅ„o identificadores legais:  
 \begin{lstlisting}
...
separados por espaÅÁo em branco. Por exemplo, a entrada \code{x+-y} ÅÈ entendida 
como a sequÅÍncia de trÅÍs ``tokens'' \code{x}, \code{+-} e \code{y}. Se
desejamos expressar a soma de \code{x} com o valor negativo de \code{y}, 
precisamos acrescentar no mÅÌnimo um espaÅÁo, ou seja, \code{x+ -y}.      
%----------
...
SeleÅÁÅıes de campo e mÅÈtodo, tal como \code{System.out.println},
 \item
%% function applications, such as \code{sqrt(x)},
aplicaÅÁÅıes de funÅÁÅ„o, tal como \code{sqrt(x)},
 \item
%% operator applications, such as \code{-x} or \code{y + x},
aplicaÅÁÅıes de operador, tais como \code{-x} ou \code{y + x},
 \item
...
funÅÁÅıes anÅÙnimas, tais como \code{x => x + 1} ou \code{(x: Int, y: Int) => x + y}.
 \end{itemize}
%----------

%----------
%% \subsection*{Definitions}:
\subsection*{DefiniÅÁÅıes:}
\begin{lstlisting}
Def          =  FunDef  |  ValDef
...
\end{lstlisting}
%%Definitions can be:
DefiniÅÁÅıes podem ser:
\begin{itemize}
\item
%%function definitions such as \code{def square(x: Int): Int = x * x},
definiÅÁÅıes de funÅÁÅ„o tal como \code{def square(x: Int): Int = x * x},
\item
%%value definitions such as \code{val y = square(2)}.
definiÅÁÅıes de valor tal como \code{val y = square(2)}.
\end{itemize}
%----------
...
Scala nÅ„o tem um tipo primitivo para nÅ˙meros racionais, mas ÅÈ fÅ·cil 
definir um, usando uma classe. Aqui estÅ· uma possÅÌvel implementaÅÁÅ„o.
...
construtores \code{n} e \code{d}, contendo as partes numÅÈricas do numerador
e denominador. A classe provÅÍ campos que retornam estas partes, bem como
mÅÈtodos para a aritmÅÈtica sobre nÅ˙meros racionais. Cada mÅÈtodo aritmÅÈtico 
recebe como parÅ‚metro o operando direito da operaÅÁÅ„o. O operando esquerdo
da operaÅÁÅ„o sempre ÅÈ o nÅ˙mero racional do mÅÈtodo cujo ÅÈ membro.   
%----------
...
A implementaÅÁÅ„o dos nÅ˙meros racionais define um mÅÈtodo privado \code{gcd} que 
computa o maior denominador comum de dois inteiros, bem como um campo
privado \code{g} que contÅÈm o \code{gcd} dos argumentos construtores. Esses
membros sÅ„o inacessÅÌveis de fora da classe \code{Rational}. SÅ„o usados na 
implementaÅÁÅ„o da classe para eliminar fatores comuns nos argumentos construtores
...
Como um exemplo de como os nÅ˙meros racionais podem ser usados, aqui estÅ· um programa 
que imprime a soma de todos os nÅ˙meros $1/i$ onde $i$ estÅ· no intervalo de 1 a 10. 
\begin{lstlisting}
var i = 1
...
operando direito um valor de tipo arbitrÅ·rio. Retorna o resultado de 
converter seu operando direito em uma cadeia de caracteres e concatenÅ·-la 
...
\paragraph{HeranÅÁa e Sobrecarga}
Cada classe em Scala tem uma superclasse que ela estende.
\comment{A exceÅÁÅ„o ÅÈ a classe \code{Object}, que nÅ„o possui superclasse, e
que ÅÈ indiretamente estendida para cada outra classe.}
Se uma classe nÅ„o menciona sua superclasse em sua definiÅÁÅ„o, o tipo bÅ·sico 
\code{scala.AnyRef} ÅÈ implicitamente assumido (para implementaÅÁÅıes Java, este
tipo ÅÈ um apelido (alias) para \code{java.lang.Object}. Por exemplo, a classe 
...
Uma classe herda todos os membros de sua superclasse. Pode tambÅÈm redefinir
(ou: {\em sobrescrever}) alguns membros herdados. Por exemplo, a classe 
\code{java.lang.Object} define um mÅÈtodo \code{toString} que retorna uma 
representaÅÁÅ„o do objeto como cadeia de caracteres (string): 
\begin{lstlisting}
class Object {
  ...
  def toString: String = ...
...
%% numbers:
A implementaÅÁÅ„o de \code{toString} em \code{Object} forma uma string que 
consiste do nome da classe do objeto e um nÅ˙mero. Faz sentido redefinir este
mÅÈtodo para objetos que sÅ„o nÅ˙meros racionais:  
\begin{lstlisting}
class Rational(n: Int, d: Int) extends AnyRef {
  ... // como antes
...
Observe que, diferentemente de Java, definiÅÁÅıes que redefinem precisam
...
Se a classe $A$ estende a classe $B$, entÅ„o objetos do tipo $A$ podem ser
usados sempre que objetos do tipo $B$ sÅ„o esperados. Nesse caso dizemos que o 
tipo $A$ estÅ· em {\em conformidade} com o tipo $B$. Por exemplo, \code{Rational}
estÅ· em conformidade com \code{AnyRef}, logo ÅÈ legal atribuir um valor \code{Rational}
Å‡ variÅ·vel do tipo \code{AnyRef}:
...
\paragraph{MÅÈtodos sem ParÅ‚metros}
...
Distintamente de Java, mÅÈtodos em Scala nÅ„o necessariamente precisam 
receber uma lista de parÅ‚metros. Um exemplo ÅÈ o mÅÈtodo abaixo \code{square}.
Este mÅÈtodo ÅÈ invocado simplesmente mencionando seu nome. 
\begin{lstlisting}
class Rational(n: Int, d: Int) extends AnyRef {
  ... // como antes
...
Ou seja, mÅÈtodos sem parÅ‚metros sÅ„o acessados como campos valor, tais como 
\code{numer} sÅ„o. A diferenÅÁa entre valores e mÅÈtodos sem parÅ‚metros 
reside em suas definiÅÁÅıes. O lado direito de um valor ÅÈ avaliado quando o 
objeto ÅÈ criado, e o valor nÅ„o muda depois. Um lado direito de um mÅÈtodo sem
parÅ‚metros, por outro lado, ÅÈ avaliado cada vez que o mÅÈtodo ÅÈ chamado. O 
acesso uniforme dos campos de mÅÈtodos sem parÅ‚metros resulta em crescente
flexibilidade para o implementador de uma classe. Frequentemente, um campo
em uma versÅ„o da classe torna-se um valor computado na prÅÛxima versÅ„o. 
O acesso uniforme garante que clientes nÅ„o tenham de ser reescritos 
por conta desta mudanÅÁa.
%----------
...
%% given by:
Considere a tarefa de escrever uma classe para conjuntos de nÅ˙meros inteiros com 
duas operaÅÁÅıes, \code{incl} e \code{contains}. \code{(s incl x)} deve retornar
um novo conjunto que contÅÈm o elemento \code{x} juntamente com todos os elementos 
do conjunto \code{s}. \code{(s contains x)} deve retornar \code{true} se o conjunto 
\code{s} contiver o elemento \code{x}, e deve retornar \code{false}, caso contrÅ·rio.
A interface para tais conjuntos ÅÈ dada por:
\begin{lstlisting}
...
%% \end{exercise}
\begin{exercise}
Escreva os mÅÈtodos \code{uniao} e \code{intersecao} para formar a uniÅ„o e interseÅÁÅ„o entre
dois conjuntos.
\end{exercise}
 \begin{exercise} Adicione um mÅÈtodo  
 \begin{lstlisting}
 def excl(x: Int)
 \end{lstlisting}
%% to return the given set without the element \code{x}. To accomplish this,
%% it is useful to also implement a test method
para retornar um dado conjunto sem o elemento \code{x}. Para isso, ÅÈ interessante tambÅÈm 
implementar um mÅÈtodo teste
 \begin{lstlisting}
 def isEmpty: Boolean
 \end{lstlisting}
%% for sets.
...
Linguagens orientadas a objetos (inclusive Scala) usam \emph{despacho dinÅ‚mico} para 
invocaÅÁÅıes de mÅÈtodos. Ou seja, o cÅÛdigo invocado por uma chamada de mÅÈtodo depende
do tipo em tempo de execuÅÁÅ„o do objeto que contÅÈm o mÅÈtodo. Por exemplo, considere
a expressÅ„o \code{s contains 7} onde \code{s} ÅÈ um valor do tipo declarado \code{s: IntSet}. 
Qual cÅÛdigo para \code{contains} ÅÈ executado depende do tipo do valor de \code{s} em 
tempo de execuÅÁÅ„o. Se for um valor \code{EmptySet}, ÅÈ a implementaÅÁÅ„o de \code{contains} na 
classe \code{EmptySet} que ÅÈ executada, e analogamente para valores \code{NonEmptySet}.
Este comportamento ÅÈ consequÅÍncia direta do nosso modelo de substituiÅÁÅ„o da avaliaÅÁÅ„o.
%----------
...
Envio dinÅ‚mico de mÅÈtodos ÅÈ anÅ·logo a chamadas para funÅÁÅıes de alta ordem. 
Em ambos os casos, a identidade do cÅÛdigo  a ser executado ÅÈ conhecida somente
em tempo de execuÅÁÅ„o. Essa similaridade nÅ„o ÅÈ apenas superficial. Na verdade, 
Scala representa cada valor de funÅÁÅ„o como um objeto (veja SeÅÁÅ„o~\ref{sec:functions}).
%----------
...
Na implementaÅÁÅ„o prÅÈvia de conjuntos de inteiros, conjuntos vazios eram expressos
com \code{new EmptySet}; entÅ„o um novo objeto era criado a cada vez que um valor
conjunto vazio era requerido. Podemos evitar criaÅÁÅıes desnecessÅ·rias de objetos 
definindo um valor \code{empty} uma vez e entÅ„o, usando este valor ao invÅÈs de cada 
ocorrÅÍncia de \code{new EmptySet}. Por exemplo:
 \begin{lstlisting}
...
Um problema com esse tratamento ÅÈ que uma definiÅÁÅ„o de valor tal como a anterior
nÅ„o ÅÈ uma definiÅÁÅ„o top-level legal em Scala; tem de ser parte de uma outra classe 
ou objeto. Ainda, a definiÅÁÅ„o de classe \code{EmptySet} agora parece um pouco exagerada -- 
por que definir uma classe de objetos, se somente estamos interessados em um Å˙nico objeto 
dessa classe? Um tratamento mais direto ÅÈ usar uma {\em definiÅÁÅ„o de objeto}. Aqui estÅ· 
uma alternativa mais adequada para a definiÅÁÅ„o de conjunto vazio:
\begin{lstlisting}
...
A sintaxe de uma definiÅÁÅ„o de objeto segue a sintaxe da definiÅÁÅ„o de uma 
classe; tem uma clÅ·usula opcional extends, bem como um corpo opcional. 
Assim como em classes, a clÅ·usula extends define membros herdados do objeto 
considerando que o corpo define novos membros ou sobrecarga. Entretanto, uma 
definiÅÁÅ„o de objeto denota um Å˙nico objeto somente se nÅ„o for possÅÌvel criar
outros objetos com a mesma estrutura usando \code{new}. EntÅ„o, definiÅÁÅıes de
objeto tambÅÈm carecem de parÅ‚metros construtores, que podem estar presentes
nas definiÅÁÅıes das classes.
%----------
...
DefiniÅÁÅıes de objetos podem aparecer em qualquer lugar em um programa Scala;
incluindo o top-level. Como nÅ„o hÅ· ordem fixa de execuÅÁÅ„o de entidades
top-level em Scala, pode-se questionar quando exatamente o objeto definido 
pela definiÅÁÅ„o de objeto ÅÈ criado e inicializado. A resposta ÅÈ que o objeto 
ÅÈ criado assim que seus membros sÅ„o acessados. Esta estratÅÈgia ÅÈ chamada 
{\em avaliaÅÁÅ„o preguiÅÁosa}.
%----------

%----------
%% \paragraph{Standard Classes}
\paragraph{Classes PadrÅ„o}
%% \todo{include picture}
...
Scala ÅÈ uma linguagem orientada a objetos pura. Isso significa que cada valor 
em Scala pode ser visto como um objeto. De fato, mesmo tipos primitivos tais como 
\code{int} ou \code{boolean} nÅ„o sÅ„o tratados de modo especial. SÅ„o definidos
como apelidos de tipos de classes Scala no mÅÛdulo \code{Predef}.  
\begin{lstlisting}
...
Por eficiÅÍncia, o compilador geralmente representa valor de tipo
\code{scala.Int} por inteiros de 32 bits, valores de tipo \code{scala.Boolean}
por boleanos Java etc. Mas converte essas representaÅÁÅıes especializadas para 
objetos quando requeridos, por exemplo, quando um valor primitivo \code{Int} ÅÈ 
passada a uma funÅÁÅ„o com um parÅ‚metro de tipo \code{AnyRef}. Consequentemente, a
representaÅÁÅ„o de valores primitivos ÅÈ apenas uma otimizaÅÁÅ„o, nÅ„o muda o significado
de um programa.
%----------

%----------
%% Here is a specification of class \code{Boolean}.
Aqui estÅ· a especificaÅÁÅ„o da classe \code{Boolean}.
\begin{lstlisting}
package scala
...
referÅÍncia ao tipos bÅ·sicos para boleanos ou nÅ˙meros. Uma possÅÌvel 
implementaÅÁÅ„o da claro \code{Boolean} ÅÈ dada abaixo. Esta nÅ„o ÅÈ 
a implementaÅÁÅ„o atual na biblioteca padrÅ„o Scala. Por razÅıes de 
eficiÅÍncia a implementaÅÁÅ„o padrÅ„o usa boleanos primitivos.
\begin{lstlisting}
package scala
abstract class Boolean {
...
Aqui estÅ· uma especificaÅÁÅ„o parcial da classe \code{Int}.
\begin{lstlisting}
package scala
abstract class Int extends AnyVal {
  ...
  def + (that: Long): Long
  def + (that: Int): Int         // anÅ·logo para -, *, /, %

  def << (cnt: Int): Int         // anÅ·logo para  >>, >>>
  ...
  def == (that: Long): Boolean   // anÅ·logo para !=, <, >, <=, >=
}
\end{lstlisting}
%----------
...
A classe \code{Int} pode em princÅÌpio tambÅÈm ser implementada usando
apenas objetos e classes, sem referÅÍncia a um tipo construÅÌdo para 
...
nÅ˙meros naturais, ou seja, inteiros nÅ„o negativo. Aqui estÅ· uma definiÅÁÅ„o 
...
Para implementar as operaÅÁÅıes da classe \code{Nat}, nÅÛs definimos um
sub-objeto \code{Zero} e uma subclasse \code{Succ} (para sucessor). Cada 
nÅ˙mero \code{N} ÅÈ representado como \code{N} aplicaÅÁÅıes do construtor \code{Succ}
atÅÈ zero: 
 \[
...
A implementaÅÁÅ„o do objeto \code{Zero} ÅÈ direta:
\begin{lstlisting}
...
A implementaÅÁÅ„o das funÅÁÅıes  predecessor e subtraÅÁÅ„o sobre \code{Zero} leva a 
um \code{Erro}, que aborta o programa com uma mensagem de erro.

Aqui estÅ· uma implementaÅÁÅ„o da classe sucessor: 
...
Observe a implementaÅÁÅ„o do mÅÈtodo \code{successor}. Para criar o 
sucessor de um nÅ˙mero, precisamos passar o prÅÛprio objeto como um 
argumento para o construtor \code{Succ}. O prÅÛprio objeto ÅÈ referenciado
...
As implementaÅÁÅıes de \code{+} e \code{-}, cada uma contÅÈm uma chamada 
recursiva com o argumento do costrutor como destinatÅ·rio. A recursÅ„o 
terminarÅ· assim que o destinatÅ·rio for o objeto \code{Zero} (o que ÅÈ 
garantido de acontecer eventualmente dado o modo com que os nÅ˙meros sÅ„o formados).
%----------
...
\begin{exercise}
Escreva uma implementaÅÁÅ„o \code{Integer} dos nÅ˙meros inteiros. A implementaÅÁÅ„o deve
suportar todas as operaÅÁÅıes da classe \code{Nat} e mais dois mÅÈtodos.
...
%% \end{exercise}
O primeiro mÅÈtodo deve retornar \code{true} se o nÅ˙mero for positivo. O segundo 
mÅÈtodo deve negativar o nÅ˙mero. NÅ„o utilize qualquer classe numÅÈrica padrÅ„o Scala
em sua implementaÅÁÅ„o. (Dica: HÅ· duas formas para implementar \code{Integer}. Uma
pode ou fazer uso da implementaÅÁÅ„o existente de \code{Nat}, representando um inteiro 
como um nÅ˙mero natural e um sinal. Ou pode-se generalizar a implementaÅÁÅ„o dada de 
\code{Nat} para \code{Integer}, usando as trÅÍs subclasses \code{Zero} para 0, 
\code{Succ} para nÅ˙meros positivos e \code{Pred} para nÅ˙meros negativos. 
\end{exercise}
%----------

%----------
%% \subsection*{Language Elements Introduced In This Chapter}
\subsection*{Elementos da Linguagem Introduzidos Neste CapÅÌtulo}
 \textbf{Types:}
 \begin{lstlisting}
 Type         = ...  |  ident
 \end{lstlisting}
...
Tipos podem agora ser identificadores arbitrÅ·rios que representam classes.
...
Uma expressÅ„o pode agora ser uma criaÅÁÅ„o de objeto, ou uma seleÅÁÅ„o
\code{E.m} de um membro \code{m} a partir de uma expressÅ„o valorada objeto \code{E},
ou pode ser a palavra reservada \code{this}.
%----------

%----------
%% \textbf{Definitions and Declarations:}
 \textbf{DefiniÅÁÅıes e DeclaraÅÁÅıes}
\begin{lstlisting}
...
Uma definiÅÁÅ„o pode agora ser uma definiÅÁÅ„o de classe, trait ou objeto, tal como  
\begin{lstlisting}
...
As definiÅÁÅıes \code{defs} na classe, trait ou objeto podem ser 
...
Classes abstratas e traits podem tambÅÈm conter declaraÅÁÅıes. Isso introduz
funÅÁÅıes {\em deferred} (postergadas) ou valores com seus tipos, mas nÅ„o 
dÅ„o uma implementaÅÁÅ„o. Membros deferred devem ser implementados nas subclasses 
...
\chapter{Classes Case e Casamento de PadrÅıes}
...
Digamos, queremos escrever um interpretador para expressÅıes aritmÅÈticas. Para 
deixar as coisas simples inicialmente, vamos nos restringir somente a nÅ˙meros e operaÅÁÅıes \code{+}.
Tais expressÅıes podem ser representadas como hierarquia de classes, 
com uma classe abstrata base \code{Expr} como raiz, e duas subclasses \code{Number} e \code{Sum}.
EntÅ„o, uma expressÅ„o \code{1 + (3 + 7)} ÅÈ representada como 
 \begin{lstlisting}
...
Agora, um avaliador de uma expressÅ„o como esta precisa saber de 
qual forma ela ÅÈ (ou \code{Sum} ou \code{Number}) e tambÅÈm precisa
acessar os componentes da expressÅ„o. A implementaÅÁÅ„o a seguir provÅÍ
todos os mÅÈtodos necessÅ·rios.
\begin{lstlisting}
abstract class Expr {
...
Com esta classificaÅÁÅ„o e mÅÈtodos de acesso, escrever uma funÅÁÅ„o avaliador ÅÈ simples:
\begin{lstlisting}
def eval(e: Expr): Int = {
...
Entretanto, definir todos esses mÅÈtodos dentro das classes \code{Sum} e \code{Number}
ÅÈ um tanto quanto tedioso. AlÅÈm do mais, o problema fica ainda pior se desejarmos 
adicionar novas formas de expressÅıes. Por exemplo, considere adicionar uma nova 
forma de expressÅ„o \code{Prod} para produtos. NÅ„o apenas teremos de implementar uma nova classe 
\code{Prod}, com todos os mÅÈtodos de acesso e classificaÅÁÅ„o prÅÈvios; tambÅÈm teremos de 
introduzir um novo mÅÈtodo abstrato \code{isProduct} dentro da classe \code{Expr} e 
implementar aquele mÅÈtodo na subclasse \code{Number}, \code{Sum}, e \code{Prod}.
Ter de  modificar cÅÛdigo existente quando um sistema cresce ÅÈ sempre problemÅ·tico, pois
introduz problemas de versÅ„o e manutenÅÁÅ„o.  
%----------
...
A promessa da programaÅÁÅ„o orientada a objetos ÅÈ que tais modificaÅÁÅıes 
sÅ„o desnecessÅ·rias, dado que podem ser evitadas pela reutilizaÅÁÅ„o de cÅÛdigo 
existente e nÅ„o modificado, atravÅÈs da heranÅÁa. De fato, uma decomposiÅÁÅ„o 
mais orientada a objetos para nosso problema resolve a questÅ„o. A idÅÈia ÅÈ tornar
a operaÅÁÅ„o de alto nÅÌvel \code{eval} um mÅÈtodo para cada classe expressÅ„o, ao 
invÅÈs de implementÅ·-lo como uma funÅÁÅ„o fora da hierarquia de classes expressÅıes, como 
fizemos anteriormente. Como \code{eval} ÅÈ agora um membro de todos os nÅÛs de expressÅ„o, 
quaisquer mÅÈtodos de classificaÅÁÅ„o e acesso tornam-se supÅÈrfluos, e a implementaÅÁÅ„o 
ÅÈ simplificada consideravelmente:  
\begin{lstlisting}
abstract class Expr {
  def eval: Int
}
...
AlÅÈm disso, adicionar uma nova classe \code{Prod} nÅ„o leva a qualquer mudanÅÁa ao cÅÛdigo existente:
\begin{lstlisting}
...
A conclusÅ„o que tiramos deste exemplo ÅÈ que a decomposiÅÁÅ„o orientada a objetos
ÅÈ a tÅÈcnica de escolha para a construÅÁÅ„o de sistemas que devem ser estensÅÌveis
com novos tipos de dados. Mas hÅ· tambÅÈm um outro possÅÌvel modo para estender a 
expressÅ„o exemplo. Podemos querer adicionar novas {\em operaÅÁÅıes} sobre expressÅıes.
Por exemplo, podemos querer adicionar uma operaÅÁÅ„o que imprime uma Å·rvore-expressÅ„o
para a saÅÌda padrÅ„o.  
%----------
...
%% example:
Se definimos todos os mÅÈtodos de classificaÅÁÅ„o e acesso, tal operaÅÁÅ„o pode ser 
facilmente escrita como uma funÅÁÅ„o externa. Aqui estÅ· um exemplo: 
\begin{lstlisting}
def print(e: Expr) {
...
%% to each class:
Entretanto, se optamos por uma decomposiÅÁÅ„o orientada a objetos das 
expressÅıes, precisaremos adicionar um novo procedimento \code{print} 
para cada classe: 
\begin{lstlisting}
abstract class Expr {
  def eval: Int
...
Consequentemente, decomposiÅÁÅ„o orientada a objetos clÅ·ssica requer modificaÅÁÅ„o 
...
Considere a simplificaÅÁÅ„o de expressÅıes. Por exemplo, podemos querer criar 
uma funÅÁÅ„o que reescreve expressÅıes da forma \code{a * b + a * c} para 
\code{a * (b + c)}. Esta operaÅÁÅ„o requer inspeÅÁÅ„o de mais de um nÅÛ para a 
Å·rvore de expressÅıes ao mesmo tempo. Consequentemente, nÅ„o pode ser implementada
por um mÅÈtodo dentro de cada tipo de expressÅ„o, a nÅ„o ser que tal mÅÈtodo tambÅÈm 
possa inspecionar outros nÅÛs. Portanto somos forÅÁados a ter mÅÈtodos de acesso e 
classificaÅÁÅ„o neste caso. Isto parece nos levar para a casa inicial, com todos os 
...
Olhando mais de perto, observa-se que o Å˙nico propÅÛsito das funÅÁÅıes de acesso
e classificaÅÁÅ„o  ÅÈ {\em reverter} o processo de construÅÁÅ„o de dados. Elas nos 
permitem determinar, primeiro, qual subclasse de uma classe base abstrata foi 
usada e, segundo, quais foram os argumentos construtores. Como essa situaÅÁÅ„o ÅÈ 
bem comum, Scala tem um modo de automatizÅ·-lo para classes case.
%----------
...
{\em Classes Case} e {\em objetos case} sÅ„o definidos como classes e objetos 
normais, exceto que a definiÅÁÅ„o ÅÈ prefixada com um modificador \code{case}. 
Por exemplo, as definiÅÁÅıes:
...
O modificador \code{case} em frente a uma definiÅÁÅ„o de classe ou objeto tem 
os seguintes efeitos.
\begin{enumerate}
\item Classes case implicitamente vem com uma funÅÁÅ„o construtora, com o mesmo nome da classe.
No nosso exemplo, as duas funÅÁÅıes
\begin{lstlisting}
...
serÅ„o adicionadas. Consequentemente, pode-se agora construir Å·rvores-expressÅıes de modo um 
...
\item Classes case e objetos case implicitamente vem com implementaÅÁÅıes dos mÅÈtodos 
\code{toString}, \code{equals} e \code{hashCode}, que sobrescrevem os
mÅÈtodos com o mesmo nome na classe \code{AnyRef}. A implementaÅÁÅ„o desses 
mÅÈtodos leva em consideraÅÁÅ„o em cada caso a estrutura de um membro de uma classe 
case. O mÅÈtodo \code{toString} representa uma Å·rvore expressÅ„o do modo como foi
construÅÌda. EntÅ„o,  
\begin{lstlisting}
Sum(Sum(Number(1), Number(2)), Number(3))
\end{lstlisting} 
...
serÅ· convertida exatamente naquela cadeia de caracteres, onde a implementaÅÁÅ„o 
default dentro da classe \code{AnyRef} retornarÅ· uma cadeia de caracteres 
consistindo construtor \code{Sum} mais externo e um nÅ˙mero. O mÅÈtodo 
\code{equals} trata dois membros case da classe case do mesmo modo, caso
eles tenham sido construÅÌdos com o mesmo construtor e com argumentos que 
sÅ„o par a par iguais. Isso tambÅÈm afeta a implementaÅÁÅ„o de \code{==} e 
\code{!=}, que sÅ„o implementados em termos de \code{equals} em Scala. EntÅ„o,  
 \begin{lstlisting}
...
darÅ· \code{true}. Se \code{Sum} ou \code{Number} nÅ„o fossem classes case, a 
mesma expressÅ„o seria \code{false}, pois a implementaÅÁÅ„o padrÅ„o de \code{equals} na 
classe \code{AnyRef} sempre trata objetos criados por diferentes chamadas de construtores 
como sendo diferentes. O mÅÈtodo \code{hashCode} segue a mesmo princÅÌpio dos 
outros dois mÅÈtodos. Computa um cÅÛdigo hash a partir do nome do construtor da classe case
e os cÅÛdigos hash dos argumentos do construtor, ao invÅÈs de o fazer a partir do endereÅÁo 
do objeto, que ÅÈ o que a implementaÅÁÅ„o default de \code{hashCode} faz.
\item
Classes case implicitamente vem com mÅÈtodos de acesso nulos que recuperam os argumentos 
do construtor. Em nosso exemplo, \code{Number} obteria um mÅÈtodo de acesso  
 \begin{lstlisting}
 def n: Int
 \end{lstlisting}
%----------
